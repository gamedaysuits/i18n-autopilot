...
async function runContentSync(options) {
  ...
  for (const sourcePath of sourceFiles) {
    ...
    for (const [, pairConfig] of pairEntries) {
      ...
      // Change detection for existing files: hash the source content and
      // compare against the stored hash from the last sync. If the source
      // changed, re-translate. If unchanged, skip.
      ...
      if (fs.existsSync(targetPath)) {
        ...
        if (storedHash && storedHash !== currentSourceHash) {
          ...
          console.log(`  [CHANGED] ${relPath} → ${code} — source content updated, re-translating`);
          retranslated++;
        } else if (!storedHash) {
          // No stored hash (legacy file or first run with manifest).
          // Store the current hash and skip — don't re-translate existing files
          // without evidence they're stale. User can delete target to force.
      ...
      // Record the source hash so future syncs can detect changes
      updatedManifest[manifestKey] = currentSourceHash;
    }
  }
