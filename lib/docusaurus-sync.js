    ...
    if (totalWork === 0) {
      ...
      await pMap(workItems, async (item) => {
        ...
        try {
          ...
          if (hasFrontMatter) {
            ...
            if (Object.keys(fieldsToTranslate).length > 0) {
              if (apiKey) {
                ...
                // No API key — only write [EN] fallback if explicitly opted in.
                if (useFallback) {
                  for (const field of fieldsList) {
                    if (frontMatter[field] && typeof frontMatter[field] === 'string') {
                  ...
                  throw new Error(
                    ...
                    '  or pass --fallback to write [EN]-prefixed placeholders.'
                  );
                }
              }
            }
          ...
          if (bodyUsedFallback) {
            // Body translation returned null from the API (network error, quota,
            // corrupted output). This is NOT the missing-key case (that throws
            // above). Record as fallback so we retry on next sync.
