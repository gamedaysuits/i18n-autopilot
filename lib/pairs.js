...
import { DEFAULT_REGISTERS, getLanguageCard, resolveCode } from './registers.js';
...
const PAIR_DEFAULTS = {
  ...
  maxRetries: 3,     // max cascade retries on batch parse failure (batch → half → individual)
};
 ...
 *
 * @param {import('./types.js').RosettaConfig} config - Resolved config (post-migration, post-defaults)
...
function resolvePairs(config) {
  ...
  for (const [code, langConfig] of Object.entries(languages)) {
    ...
    const model = langConfig.model || defaultModel;
    ...
    const register = langConfig.register || registerInfo.register || 'Professional register.';
    if (!langConfig.register) _defaults.add('register');

    pairs.set(pairKey, {
      source: inputLocale,
      target: code,
      method: langConfig.method || defaultMethod,
      model,
      ...
      dir: card?.dir || registerInfo.dir || 'ltr',
      ...
      promptContext: config.promptContext || null,
      _defaults,
    });
  }
  ...
  if (config.pairs && typeof config.pairs === 'object') {
    for (const [rawPairKey, pairOverride] of Object.entries(config.pairs)) {
      ...
      // Canonicalize locale codes through alias resolution (e.g., fil → tl,
      // no → nb, iw → he) so that pair overrides merge correctly with
      // pairs created from the languages config in Step 1.
      const source = resolveCode(rawSource);
      const target = resolveCode(rawTarget);
      ...
      const _defaults = new Set();
      const model = pairOverride.model || existing.model || defaultModel;
      ...
      const register = pairOverride.register || existing.register || registerInfo.register || 'Professional register.';
      ...
      pairs.set(pairKey, {
        source,
        target,
        method: pairOverride.method || existing.method || defaultMethod,
        model,
        ...
        dir: card?.dir || existing.dir || registerInfo.dir || 'ltr',
        ...
        promptContext: config.promptContext || existing.promptContext || null,
        _defaults,
      });
    }
  }
