...
import { DEFAULT_MODEL, DEFAULT_BATCH_SIZE, DEFAULT_MAX_RETRIES } from './config.js';
...
function resolvePairs(config) {
  ...
  for (const [code, langConfig] of Object.entries(languages)) {
    ...
    pairs.set(pairKey, {
      ...
      temperature: langConfig.temperature ?? config.temperature ?? null,
      _defaults,
    });
  }
  ...
  if (config.pairs && typeof config.pairs === 'object') {
    for (const [rawPairKey, pairOverride] of Object.entries(config.pairs)) {
      ...
      pairs.set(pairKey, {
        ...
        temperature: pairOverride.temperature ?? existing.temperature ?? config.temperature ?? null,
        _defaults,
      });
    }
  }
