...
// -----------------------------------------------------------------
// LanguageCard — runtime-tier language metadata from language-cards/*.json
// Produced by: registers.js:getLanguageCard()
// Consumed by: config.js, pairs.js, init.js, status.js, deepl.js
//
// NOTE (v5 Two-Tier Architecture):
//   getLanguageCard() returns ONLY runtime fields (translation config).
//   For reference-tier data (linguisticChallenges, encyclopedic, resources),
//   use getLanguageReference() — it lazy-loads from language-reference/*.json
//   and returns a merged runtime + reference object.
// -----------------------------------------------------------------
 ...
 * @property {string[]} evalDatasets - Dataset IDs for the companion eval harness (not consumed by rosetta at runtime)
