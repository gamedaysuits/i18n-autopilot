...
class DirectLLMMethod extends LLMMethod {
  ...
  async translate(keys, sourceFlat, pairConfig, options) {
    ...
    const batchFn = (batch, opts) => this._callProviderBatch(batch, { ...opts, apiKey, model, coaching });
  ...
  async _callProviderBatch(toTranslate, options) {
    const { apiKey, model, batchNum, systemMessage, coaching } = options;
    ...
    if (coaching && coaching.dictionary) {
      ...
      prompt = buildUserMessage(toTranslate);
    }
