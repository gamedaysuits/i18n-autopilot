...
class DirectLLMMethod extends LLMMethod {
  ...
  async translateContent(prompt, pairConfig, options) {
    ...
    console.error(`\n     ⟳ ${label}: standard retries exhausted — escalating with extended timeout...`);
    await sleep(10_000);
      ...
      try {
        ...
        if (isRetryable(response.status)) {
          if (attempt < MAX_RETRIES) {
            ...
            console.error(`\n     ⏳ ${label}: ${response.status} — retry ${attempt + 1}/${MAX_RETRIES} in ${Math.round(delay / 1000)}s...`);
            await sleep(delay);
            continue;
          }
        ...
        if (!content) {
          ...
          if (attempt < MAX_RETRIES) {
            const delay = getBackoffDelay(attempt);
            console.error(`\n     ⏳ ${label}: empty response — retry ${attempt + 1}/${MAX_RETRIES} in ${Math.round(delay / 1000)}s...`);
            await sleep(delay);
            continue;
          }
        ...
        if (attempt < MAX_RETRIES) {
          const delay = getBackoffDelay(attempt);
          console.error(`\n     ⏳ ${label}: ${errLabel} — retry ${attempt + 1}/${MAX_RETRIES} in ${Math.round(delay / 1000)}s...`);
          await sleep(delay);
          continue;
        }
