...
import { DEFAULT_OPENROUTER_MODEL, DEFAULT_BATCH_SIZE, DEFAULT_COACHED_TEMPERATURE, DEFAULT_MAX_RETRIES } from '../config.js';
...
class LLMCoachedMethod extends TranslationMethod {
  ...
  async translate(keys, sourceFlat, pairConfig, options) {
    ...
    const batchFn = (batch, opts) => this._callCoachedBatch(batch, coaching, opts, pairConfig);

    // ── Parallel batch execution ───────────────────────────────────────
    // Same pattern as LLMMethod.translate() — fire batch chunks
    // concurrently, each cascading independently on failure.
    const batchChunks = [];
