...
class LLMCoachedMethod extends TranslationMethod {
  ...
  async translate(keys, sourceFlat, pairConfig, options) {
    ...
    for (let i = 0; i < keys.length; i += batchSize) {
      ...
      // Build per-batch user message with dictionary hints specific to this batch
      const result = await this._translateCoachedWithCascade(toTranslate, coaching, {
        apiKey,
        model,
        batchNum,
        maxRetries,
  ...
  async estimateCost(keyCount, pairConfig = {}) {
    const model = pairConfig.model || 'openai/gpt-4o-mini';
  ...
  /**
   * Retry cascade for a coached batch — mirrors LLMMethod._translateWithCascade.
   *
   * Uses the same split-and-retry strategy but with per-batch dictionary
   * hint injection in the user message.
   */
  async _translateCoachedWithCascade(toTranslate, coaching, options) {
    const { maxRetries, systemMessage } = options;
    let retriesUsed = 0;

    // Attempt 1: Full batch
    const result = await this._callCoachedBatch(toTranslate, coaching, options);

    if (result && !result._parseError) return result;
    if (!result) return null;

    // Parse error — start cascade
    retriesUsed++;
    if (retriesUsed > maxRetries) {
      this._logCascadeExhausted(toTranslate, options);
      return null;
    }
    ...
    if (keys.length > 1) {
      ...
      console.error(`\n     ⟳ Coached batch ${options.batchNum}: parse error — splitting into halves...`);

      const merged = {};
      for (const half of [firstHalf, secondHalf]) {
        retriesUsed++;
        if (retriesUsed > maxRetries) {
          this._logCascadeExhausted(half, options);
          break;
        }

        const halfResult = await this._callCoachedBatch(half, coaching, options);
        ...
        if (halfResult && !halfResult._parseError) {
          ...
          for (const [key, value] of Object.entries(half)) {
            retriesUsed++;
            if (retriesUsed > maxRetries) {
              console.error(`     [ERR] Coached retry budget exhausted (${maxRetries}).`);
              break;
            }
            const singleResult = await this._callCoachedBatch({ [key]: value }, coaching, options);
            if (singleResult && !singleResult._parseError) {
              Object.assign(merged, singleResult);
            } else {
              console.error(`     [ERR] Key "${key}" failed even as individual coached translation.`);
            }
          }
        }
      }
    ...
    const retryResult = await this._callCoachedBatch(toTranslate, coaching, options);
   ...
   */
  _logCascadeExhausted(toTranslate, options) {
    const keys = Object.keys(toTranslate);
    console.error(`\n     [ERR] Coached batch ${options.batchNum}: retry cascade exhausted (${options.maxRetries} max).`);
