  ...
  async _translateSingleText(text, sourceLocale, targetLocale, apiKey) {
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        const timeoutId = setTimeout(() => controller.abort(), GOOGLE_REQUEST_TIMEOUT_MS * 2);
        ...
        if (isRetryable(response.status)) {
          if (attempt < MAX_RETRIES) {
            ...
            console.error(`\n     ⏳ Google Translate content: ${response.status} — retrying in ${Math.round(delay / 1000)}s...`);
            await sleep(delay);
            continue;
          }
          console.error(`\n     [ERR] Google Translate content: ${response.status} after ${MAX_RETRIES + 1} attempts`);
          return null;
        }
        ...
        if (err.name === 'AbortError') {
          console.error('\n     ⏳ Google Translate content: Timeout — retrying...');
        } else if (attempt < MAX_RETRIES) {
          const delay = getBackoffDelay(attempt);
          console.error(`\n     ⏳ Google Translate content: ${err.message} — retrying in ${Math.round(delay / 1000)}s...`);
          await sleep(delay);
        } else {
          console.error(`\n     [ERR] Google Translate content: ${err.message} after ${MAX_RETRIES + 1} attempts`);
          return null;
        }
      }
    }
