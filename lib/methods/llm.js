...
class LLMMethod extends TranslationMethod {
   ...
   */
  async _translateWithCascade(toTranslate, langConfig, options) {
    const { maxRetries, systemMessage } = options;
    let retriesUsed = 0;

    // Attempt 1: Full batch
    const result = await this._callOpenRouterBatch(toTranslate, options);
    ...
    if (keys.length > 1) {
      ...
      console.error(`\n     ⟳ Batch ${options.batchNum}: parse error — splitting ${keys.length} keys into halves (${Object.keys(firstHalf).length} + ${Object.keys(secondHalf).length})...`);
      ...
      for (const half of [firstHalf, secondHalf]) {
        ...
        const halfResult = await this._callOpenRouterBatch(half, options);
        ...
        if (halfResult && !halfResult._parseError) {
          ...
          console.error(`     ⟳ Half-batch parse error — falling back to individual keys...`);
          for (const [key, value] of Object.entries(half)) {
            retriesUsed++;
            if (retriesUsed > maxRetries) {
              console.error(`     [ERR] Retry budget exhausted (${maxRetries}). Skipping remaining keys.`);
              console.error(`           Consider: "pairs": { "${options.pairKey || 'en:' + key.split('.')[0]}": { "model": "google/gemini-3.1-pro-thinking", "batchSize": 5 } }`);
              break;
            }

            const singleResult = await this._callOpenRouterBatch({ [key]: value }, options);
            if (singleResult && !singleResult._parseError) {
              Object.assign(merged, singleResult);
            } else {
              console.error(`     [ERR] Key "${key}" failed even as individual translation.`);
            }
          }
        }
    ...
    const retryResult = await this._callOpenRouterBatch(toTranslate, options);
  ...
  _logCascadeExhausted(toTranslate, options) {
    const keys = Object.keys(toTranslate);
    console.error(`\n     [ERR] Batch ${options.batchNum}: retry cascade exhausted (${options.maxRetries} max).`);
