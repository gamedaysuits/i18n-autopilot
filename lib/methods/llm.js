...
class LLMMethod extends TranslationMethod {
  ...
  async translate(keys, sourceFlat, pairConfig, options) {
    ...
    for (let i = 0; i < keys.length; i += batchSize) {
      ...
      // Fire progress callback after each batch chunk completes.
      // Gives consumers (sync.js) per-batchSize updates for the progress bar.
      if (options.onProgress) {
        options.onProgress(
          Math.min(i + batchSize, keys.length),
          keys.length,
        );
      }
    }
