...
import { runContentSync, CONTENT_LOCK_FILENAME } from './content-sync.js';
            ...
            if (!useFallback) {
              ...
              if (method === 'llm' || method === 'llm-coached') {
                ...
                if (!dKey) {
                  console.error('');
                  console.error('  ┌─ Missing API Key ─────────────────────────────────────────────┐');
                  console.error('  │ DeepL requires an API authentication key.                      │');
                  console.error('  │                                                                │');
                  console.error('  │ 1. Sign up at https://www.deepl.com/pro-api (free tier avail.) │');
                  console.error('  │ 2. Run: export DEEPL_API_KEY=...                               │');
                  console.error('  │ 3. Or add to .env.local: DEEPL_API_KEY=...                     │');
                  console.error('  │                                                                │');
                  console.error('  │ Free keys end with \":fx\" — detected automatically.              │');
                  console.error('  └────────────────────────────────────────────────────────────────┘');
                } else {
                  console.error('        API key is set but translation failed. Check your DeepL dashboard for quota/billing.');
                }
              } else if (method === 'microsoft-translator') {
                const msKey = process.env.MICROSOFT_TRANSLATOR_API_KEY || process.env.AZURE_TRANSLATOR_KEY;
                if (!msKey) {
                  console.error('');
                  console.error('  ┌─ Missing API Key ─────────────────────────────────────────────┐');
                  console.error('  │ Microsoft Translator requires an Azure subscription key.       │');
                  console.error('  │                                                                │');
                  console.error('  │ 1. Create a Translator resource in Azure Portal                │');
                  console.error('  │ 2. Run: export MICROSOFT_TRANSLATOR_API_KEY=...                │');
                  console.error('  │ 3. Set region: export MICROSOFT_TRANSLATOR_REGION=eastus        │');
                  console.error('  │                                                                │');
                  console.error('  │ Docs: https://learn.microsoft.com/azure/cognitive-services/     │');
                  console.error('  │       translator/quickstart-text-rest-api                       │');
                  console.error('  └────────────────────────────────────────────────────────────────┘');
                } else {
                  console.error('        API key is set but translation failed. Check your Azure Portal for quota/billing.');
                }
              } else if (method === 'libretranslate') {
                console.error('');
                console.error('  ┌─ LibreTranslate Connection Failed ──────────────────────────────┐');
                console.error('  │ Could not connect to LibreTranslate API.                         │');
                console.error('  │                                                                  │');
                console.error('  │ Default endpoint: http://localhost:5000/translate                 │');
                console.error('  │                                                                  │');
                console.error('  │ To use a remote instance:                                        │');
                console.error('  │   export LIBRETRANSLATE_API_URL=https://your-instance/translate   │');
                console.error('  │   export LIBRETRANSLATE_API_KEY=... (if required)                 │');
                console.error('  │                                                                  │');
                console.error('  │ Self-host: docker run -p 5000:5000 libretranslate/libretranslate  │');
                console.error('  └──────────────────────────────────────────────────────────────────┘');
                ...
                if (!oaiKey) {
                  console.error('');
                  console.error('  ┌─ Missing API Key ─────────────────────────────────────────────┐');
                  console.error('  │ The OpenAI method requires an OpenAI API key.                  │');
                  console.error('  │                                                                │');
                  console.error('  │ 1. Sign up at https://platform.openai.com                      │');
                  console.error('  │ 2. Run: export OPENAI_API_KEY=sk-...                           │');
                  console.error('  │ 3. Or add to .env.local: OPENAI_API_KEY=sk-...                 │');
                  console.error('  └────────────────────────────────────────────────────────────────┘');
                } else {
                  console.error('        API key is set but translation failed. Check your OpenAI dashboard for quota/billing.');
                }
                ...
                if (!antKey) {
                  console.error('');
                  console.error('  ┌─ Missing API Key ─────────────────────────────────────────────┐');
                  console.error('  │ The Anthropic method requires an Anthropic API key.             │');
                  console.error('  │                                                                │');
                  console.error('  │ 1. Sign up at https://console.anthropic.com                     │');
                  console.error('  │ 2. Run: export ANTHROPIC_API_KEY=sk-ant-...                    │');
                  console.error('  │ 3. Or add to .env.local: ANTHROPIC_API_KEY=sk-ant-...          │');
                  console.error('  └────────────────────────────────────────────────────────────────┘');
                } else {
                  console.error('        API key is set but translation failed. Check your Anthropic console for quota/billing.');
                }
                ...
                if (!gemKey) {
                  console.error('');
                  console.error('  ┌─ Missing API Key ─────────────────────────────────────────────┐');
                  console.error('  │ The Gemini method requires a Google AI API key.                 │');
                  console.error('  │                                                                │');
                  console.error('  │ 1. Get a key at https://aistudio.google.com/apikey              │');
                  console.error('  │ 2. Run: export GEMINI_API_KEY=...                              │');
                  console.error('  │ 3. Or add to .env.local: GEMINI_API_KEY=...                    │');
                  console.error('  └────────────────────────────────────────────────────────────────┘');
                } else {
                  console.error('        API key is set but translation failed. Check your Google AI Studio for quota/billing.');
                }
              } else {
...
async function runDocusaurusSync(options, config, cwd) {
  ...
  if (contentSources.length === 0) {
    ...
    for (const { dir: sourceContentDir, plugin: pluginName } of contentSources) {
      ...
      for (const sourcePath of sourceFiles) {
        ...
        for (const [, pairConfig] of pairEntries) {
          ...
          // Change detection: hash source content and compare against manifest.
          // Reuses the same lock file as Hugo content sync.
          const docuManifestKey = `docusaurus:${dirName}/${relPath}:${code}`;
          const docuSourceHash = crypto.createHash('sha256')
            .update(fs.readFileSync(sourcePath, 'utf-8'), 'utf-8').digest('hex');

          if (fs.existsSync(targetPath)) {
            const storedHash = docuContentManifest[docuManifestKey];
            if (storedHash && storedHash === docuSourceHash) {
              totalContentSkipped++;
              continue;
            }
            if (storedHash && storedHash !== docuSourceHash) {
              console.log(`    [CHANGED] ${dirName}/${relPath} → ${code} — source updated, re-translating`);
              totalContentRetranslated++;
            } else if (!storedHash) {
              // Legacy file, no stored hash — store and skip
              updatedDocuManifest[docuManifestKey] = docuSourceHash;
              totalContentSkipped++;
              continue;
            }
          }
          ...
          // Record source hash for future change detection
          updatedDocuManifest[docuManifestKey] = docuSourceHash;
        }
      }
    }

    // Persist content manifest (shared with Hugo content sync)
    if (!dryRun) {
      const sorted = {};
    ...
    if (totalCreated > 0 || totalContentSkipped > 0 || totalContentRetranslated > 0) {
      ...
      console.log(`\n  [OK] ${action} ${totalCreated} content file(s)${fallbackNote}${retranslateNote}, ${totalContentSkipped} unchanged`);
    }
  }
