        ...
        for (const [, pairConfig] of pairEntries) {
          ...
          if (fs.existsSync(targetPath)) {
            ...
            if (storedHash && storedHash !== docuSourceHash) {
              ...
              if (!isRosettaFallback) {
                // Genuine hand-translated or externally-translated file.
                // Record its hash so future runs can detect source changes,
                // but don't overwrite the existing translation.
                updatedDocuManifest[docuManifestKey] = docuSourceHash;
                totalContentSkipped++;
                continue;
              }
              // Rosetta-generated fallback — fall through to re-translate
              console.log(`    [RE-TRANSLATE] ${dirName}/${relPath} → ${code} — replacing [EN] fallback`);
            }
          }
          ...
          if (body.trim()) {
            ...
            if (apiKey) {
              ...
              translatedBody = `${commentStart}[EN] Original English content${commentEnd}\n${body}`;
              // NOTE: do NOT increment totalContentFallback here — the canonical
              // accounting happens after the file is written (see below) to avoid
              // double-counting. This branch sets the body content; the post-write
              // block handles the file-level counter.
            }
          }
          ...
          updatedDocuManifest[docuManifestKey] = docuSourceHash;
            ...
            // Per-file error boundary: log and continue with the next pair/file
            // instead of aborting the entire Phase 2 content sync.
