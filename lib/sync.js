 ...
 *   1. Prints version banner (e.g., "i18n-rosetta v3.3.2")
...
async function runSync(options = {}) {
  ...
  const localeResults = await pMap(pairEntries, async ([pairKey, pairConfig]) => {
      ...
      if (!dryRun) {
        ...
        if (stringKeys.length > 0) {
          ...
          if (translated) {
            ...
            return { processed: 0, tmHits: localeTMHits };
    ...
    return { processed: localeProcessed, tmHits: localeTMHits };
  ...
  if (!dryRun && !audit && !cliArgs['no-verify']) {
    await verifyLocales(config, cwd);
  }
}
