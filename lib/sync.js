 ...
 *
 *   GOD-MODULE RISK: This file is ~1000 lines and handles config resolution,
 *   format detection, hash manifest management, pair graph iteration, cost
 *   estimation, translation dispatch, quality gating, script conversion,
 *   file I/O, and content sync. Known extraction candidates:
 *     1. Cost estimation (~70 lines) → lib/cost-report.js
 *     2. Per-pair translation loop (~250 lines) → translatePair() function
 *     3. Docusaurus sync (~400 lines) → lib/docusaurus-sync.js
 *   Rule: if adding a feature, consider a helper module before inlining here.
 *
 *   OUTPUT ADOPTION GAP: This module uses console.log/console.error directly
 *   (~40 call sites). The output controller (lib/output.js) supports --json
 *   and --quiet modes but is not wired into this pipeline. This means:
 *     - `sync --json` does NOT produce machine-readable JSON output
 *     - `sync --quiet` does NOT suppress info-level messages
 *   Migrating to output.info/ok/warn/error is the single biggest DX
 *   improvement remaining. See lib/output.js for the API.
 */
...
import { translateBatch, isUnsafeKey, getMethod } from './translate.js';
...
import { detectFormatFromDir, getExtension, readLocaleFile, writeLocaleFile, extractDocusaurusMessages, injectDocusaurusMessages, extractDocusaurusDescriptions } from './format.js';
...
import { loadTM, saveTM, storeTM, partitionByTM, tmSize } from './tm.js';
...
async function runSync(options = {}) {
  ...
  if (noTM) {
    console.log('[INFO] Translation Memory disabled (--no-tm)');
  } else if (tmInitialSize > 0) {
    console.log(`[INFO] Translation Memory: ${tmInitialSize} cached entries loaded`);
  }
    ...
    if (diff.toProcess.length > 0) {
      ...
      if (!dryRun) {
        ...
        if (stringKeys.length > 0) {
          // Translation Memory: partition keys into cached hits and API misses.
          // Hits are reused instantly — only misses are sent to the translation API.
          const { hits: tmHits, misses: tmMisses } = partitionByTM(
            tm, sourceFlat, stringKeys, code, pairConfig.method || 'llm'
          );

          if (Object.keys(tmHits).length > 0) {
            totalTMHits += Object.keys(tmHits).length;
            console.log(`     [TM] ${Object.keys(tmHits).length} key(s) served from cache`);
          }
          ...
          if (tmMisses.length > 0) {
            ...
            process.stdout.write(`     Translating ${tmMisses.length} key(s) to ${pairConfig.name} (${pairConfig.method})...`);
            const apiResult = await translateBatch(tmMisses, sourceFlat, pairConfig, {
              apiKey,
            ...
            if (apiResult) {
              ...
              // Store new translations in TM for future runs
              for (const [k, v] of Object.entries(apiResult)) {
                if (typeof v === 'string' && typeof sourceFlat[k] === 'string') {
                  storeTM(tm, sourceFlat[k], code, pairConfig.method || 'llm', v);
                }
              }
            }
          }
  ...
  if (!dryRun) {
    ...
    if (!noTM && tmFinalSize > tmInitialSize) {
      saveTM(cwd, tm);
      console.log(`[TM] Saved ${tmFinalSize - tmInitialSize} new entries (${tmFinalSize} total)`);
    }
  }
...
async function runDocusaurusSync(options, config, cwd) {
  ...
  if (forceContent) {
    ...
    if (fs.existsSync(contentLockPath)) {
      ...
      console.log('  [INFO] Cleared content lock file — all content will be re-translated.');
    }
  }
  ...
  if (noTM) {
    ...
    console.log(`  [TM] ${tmInitialSize} cached entries loaded`);
  }
    ...
    for (const [pairKey, pairConfig] of pairEntries) {
      ...
      if (diff.toProcess.length > 0) {
        ...
        if (!dryRun) {
          ...
          if (stringKeys.length > 0) {
            // TM partition: serve cached hits, only send misses to the API
            const { hits: tmHits, misses: tmMisses } = partitionByTM(
              tm, sourceFlat, stringKeys, code, pairConfig.method || 'llm'
            );
            ...
            if (tmMisses.length > 0) {
              ...
              if (apiResult) {
                ...
                // Store new API translations in TM
                for (const [k, v] of Object.entries(apiResult)) {
                  if (typeof v === 'string' && typeof sourceFlat[k] === 'string') {
                    storeTM(tm, sourceFlat[k], code, pairConfig.method || 'llm', v);
                  }
            ...
            } else if (tmMisses.length > 0) {
              // API was called but returned nothing, and no TM hits either
              if (!useFallback) {
  ...
  if (contentSources.length === 0) {
    ...
    if (totalWork === 0) {
      console.log('  [OK] All content files are up to date.');
    } else {
      ...
      await pMap(workItems, async (item) => {
        ...
        try {
          ...
          if (dryRun) {
            ...
            console.log(`    [DRY] ${dirName}/${relPath} → ${code}`);
            totalContent++;
            return;
          }
          ...
          console.error(`    [ERR] ${dirName}/${relPath} → ${code} — ${contentErr.message}`);
        }
        ...
        console.log(`    [${completed}/${totalWork}] (${pct}%) ${dirName}/${relPath} → ${code} [${tag}]${etaStr}`);
    ...
    if (contentFallbacks.length > 0) {
      console.log(`\n  ⚠  ${contentFallbacks.length} content file(s) used [EN] fallback (will retry on next sync):`);
      const shown = contentFallbacks.slice(0, 5);
      for (const fb of shown) {
        console.log(`     - ${fb}`);
      }
      if (contentFallbacks.length > 5) {
        console.log(`     ... and ${contentFallbacks.length - 5} more`);
      }
