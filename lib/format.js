...
function writeLocaleFile(filePath, data, format, flatData, yamlStyle) {
  if (format === 'toml') {
    ...
    fs.writeFileSync(filePath, serializer(flatData || data), 'utf-8');
  } else {
...
function detectYAMLStyle(content) {
  ...
  // Check if ALL sub-keys across ALL sections are CLDR plural forms.
  // If any section has a sub-key that is NOT a plural form, it's standard nested.
  for (const [, values] of sectionEntries) {
    for (const subKey of Object.keys(values)) {
      if (!PLURAL_FORMS.has(subKey)) {
        return 'nested';
      }
    }
  }
...
// strings that rosetta's diff/translate pipeline expects.
//
