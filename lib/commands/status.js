...
async function run(args, cwd) {
  ...
  if (pairs.size === 0) {
    ...
    // Show active registers for each pair
    output.raw('  Registers:\n');
    for (const [pairKey, pairConfig] of pairs) {
      const defaultReg = DEFAULT_REGISTERS[pairConfig.target];
      const isCustom = defaultReg && pairConfig.register !== defaultReg.register;
      const label = isCustom ? '(custom)' : '(default)';
      // Truncate long registers for readable output
      const regText = pairConfig.register || 'Professional register.';
      const truncated = regText.length > 60 ? regText.slice(0, 57) + '...' : regText;
      output.raw(`    ${pairConfig.target}  ${truncated}  ${label}`);
    }
    output.raw('');
  }
