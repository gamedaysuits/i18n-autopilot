 ...
 *   5. Model (default model from config.js)
...
import { CONFIG_FILENAMES, DEFAULT_MODEL, DEFAULT_BATCH_SIZE } from '../config.js';
...
async function runInteractive(cwd) {
  ...
  try {
    ...
    // Step 5: Model
    const model = await ask(rl, 'Translation model', DEFAULT_MODEL);
