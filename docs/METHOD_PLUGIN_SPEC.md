...
> **Version**: 1.1
> **Status**: SSOT — canonical specification for method plugin manifests.
> **Copies**: `website/docs/reference/plugin-spec.md` (Rosetta product site, with Docusaurus frontmatter).
> **JSON Schema**: `schemas/rosetta-plugin.schema.json`
...
The eval harness's job is to **develop, test, and export** translation methods. i18n-rosetta's job is to **consume and execute** them. The plugin is **data only** — configuration, coaching content, and benchmark results. No Python code, no harness dependencies.

### Data Flow

```
Evaluation Harness  ──────────►  i18n-rosetta
   (Python)           method.json     (Node.js)
                   + coaching data
```

The harness develops and tests methods in Python. When a method is ready for deployment, the harness exports a `method.json` manifest and optional coaching data files. Rosetta installs and executes the method using its own built-in method implementations.
...
**Merge semantics**: The plugin defines *what* method to use (`type`). The pair config tunes *how* to run it (`model`, `register`, `batchSize`). If the pair sets `model`, it overrides the plugin's default.
...
Plugin manifests are validated at install time against `schemas/rosetta-plugin.schema.json`.
