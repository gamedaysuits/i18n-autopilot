...
🌐 **README-vertalingen** — *uiteraard vertaald door rosetta:*
...
Rosetta detecteert automatisch uw locale-bestanden, hun formaat en de doeltalen. Het vertaalt ontbrekende sleutels, slaat reeds vertaalde over en schrijft de resultaten weg. Dat is alles.

## Waarom niet gewoon zelf scripten?

U zou een snel script kunnen schrijven dat door uw Engelse sleutels loopt en Google Translate aanroept. De meeste ontwikkelaars doen dit — het kost ongeveer 30 regels. Dit is waarom het misgaat:

- **Geen wijzigingsdetectie.** Wanneer u een Engelse string bijwerkt, blijft de vertaling voor altijd verouderd. Rosetta volgt elke bronwaarde met SHA-256 hashes en vertaalt alleen wat is gewijzigd opnieuw.
- **Geen batchverwerking.** Eén API-aanroep per sleutel betekent 200 sleutels = 200 round trips. Rosetta verwerkt intelligent in batches (configureerbaar, standaard 30 sleutels/batch voor LLM, 128 voor Google).
- **Geen kwaliteitscontrole.** Machinevertaling hallucineert, herhaalt de bron of geeft output in het verkeerde schrift. Rosetta valideert elke vertaling voordat deze wordt weggeschreven — verkeerd schrift, lengte-inflatie en bronherhalingen worden opgevangen en afgewezen.
...
- **Geen veiligheid.** Rosetta beschermt tegen prototype pollution, path traversal via speciaal gemaakte locale-codes en corruptie van codeblokken tijdens Markdown-vertaling.
...
Rosetta heeft een vertaalbackend nodig. Kies er één:
...
| **DeepL** | `DEEPL_API_KEY` | Europese talen, woordenlijst ondersteuning |
...
**OpenRouter** (200+ modellen): Meld u aan op [openrouter.ai](https://openrouter.ai), en dan:
...
> **Opmerking**: Als alleen `GOOGLE_TRANSLATE_API_KEY` is ingesteld, schakelt rosetta automatisch over naar Google Translate. Geen configuratiewijziging nodig. Maakt direct gebruik van de REST API — geen SDK, geen serviceaccount, geen `pip install`. Alleen de sleutel.
...
Elke taal wordt geleverd met **register presets** — vooraf gebouwde toon-/formaliteitsinstructies afgestemd op het taalsysteem (vouvoiement voor Frans, Siezen voor Duits, です/ます voor Japans, 해요체 voor Koreaans). De init-wizard laat u presets bekijken en kiezen, of `--yes` doorgeven om de standaardinstellingen te accepteren.
...
- **Gecached** — Vertaalgeheugen slaat eerdere resultaten op; opnieuw uitvoeren van sync kost niets voor ongewijzigde sleutels
- **Kwaliteitsgecontroleerd** — Valideert elke vertaling: vangt hallucinaties, output in verkeerd schrift, bronherhalingen en lengte-inflatie op
...
- **XLIFF interop** — Exporteer vertalingen voor professionele beoordeling in CAT-tools (memoQ, SDL Trados, Phrase), importeer ze terug
- **Geen afhankelijkheden** — Alleen Node.js built-ins. Geen SDK's, geen native modules. Vereist Node 20+

## Voorbij Google Translate

De snelle start brengt u aan de slag met een LLM of Google Translate. Maar Google Translate ondersteunt ~130 talen. Er zijn er meer dan 7.000.
...
> Geboren uit het vertalen van een productiewebsite naar Plains Cree, waar geen kant-en-klare API bestaat. De per-paar architectuur is niet theoretisch — het bestaat omdat één project Google Translate nodig had voor Frans en een gecoachte FST-pijplijn voor een inheemse taal, naast elkaar draaiend in hetzelfde sync-commando.

De bijbehorende [MT Eval Harness](https://github.com/gamedaysuits/gds-mt-eval-harness) laat u vertaalbenaderingen benchmarken en vergelijken, en vervolgens werkende methoden exporteren als rosetta-plugins. Iedereen die beide talen spreekt, kan een vertaalmethode ontwikkelen, testen en delen — geen eigen platform vereist.
...
**Traditionele MT** — het beste voor snelheid, kosten en grote hoeveelheden sleutel-waardeparen:

| Methode | Sleutel | Wat het doet |
|--------|-----|-------------|
| `google-translate` | `GOOGLE_TRANSLATE_API_KEY` | Google Cloud Translation API v2 (130+ talen) |
| `deepl` | `DEEPL_API_KEY` | DeepL API met woordenlijstondersteuning (30+ talen) |
...
| `lint` | Zoek hardcoded strings in broncode |
| `wrap` | Automatisch hardcoded strings inpakken in `t()` aanroepen (met ongedaan maken) |
| `seo` | Genereer hreflang, sitemap.xml, of JSON-LD schema |
| `integrity` | Controleer op placeholder corruptie, codering en ICU meervoud volledigheid |
...
| `fonts` | Download webfonts voor PUA script converters |
...
| `defaultMethod` | `"llm"` | Standaard vertaalmethode (overschreven door `--method` vlag) |
| `batchSize` | `30` | Sleutels per vertaalbatch |
| `pairs` | `{}` | Per-paar methode, model en kwaliteits-overschrijvingen |

**Per-taal overschrijvingen**: Elke taal heeft een [Taalkaart](docs/planning/LANGUAGE_CARD_SPEC.md) — een van de 50 samengestelde kaarten met register-presets, formaliteitssystemen, typografische regels en methode-ondersteuningsvlaggen. Kaarten gebruiken een [twee-tier architectuur](website/docs/concepts/architecture.md) (runtime + referentie) voor prestaties op schaal. Maak een nieuwe kaart met `node scripts/generate-language-card.mjs <code>`. Gebruik preset-sleutels als afkorting, of schrijf aangepaste registertekst:
...
Taalwaarden kunnen een preset-sleutel zijn (bijv. `"casual-tu"`), aangepaste registertekst, of een object (volledige controle). Overschrijvingen op paarniveau in `pairs` hebben voorrang op instellingen op taalniveau. Voer `npx i18n-rosetta init` uit om de beschikbare presets voor elke taal te bekijken.

Framework setup gidsen: [docs/INTEGRATION_GUIDES.md](https://github.com/gamedaysuits/i18n-rosetta/blob/main/docs/INTEGRATION_GUIDES.md)

## Verharding

- **Exponentiële backoff** — 3 herhalingen met jitter bij 429/5xx fouten
- **30s request timeout** — AbortController voorkomt vastlopen
- **Response validatie** — accepteert alleen sleutels die zijn verzonden voor vertaling
- **Kwaliteitscontrole** — vangt hallucinatie-loops, output in verkeerd schrift, lengte-inflatie en bronherhalingen op
- **Retry cascade** — bij JSON parse-fout, herhaalt batch → halve batch → individuele sleutels (budget-capped via `maxRetries`)
- **Vertaalgeheugen** — `.rosetta/tm.json` cachet vertalingen gesleuteld op brontekst + locale + methode; ongewijzigde sleutels worden uit de cache geserveerd bij volgende synchronisaties, waardoor redundante API-aanroepen worden geëlimineerd
- **Prompt caching** — systeem/gebruiker bericht splitsing maakt caching op providerniveau mogelijk, waardoor de tokenkosten over batches worden verlaagd
- **Terminologiehandhaving** — gecoachte vertalingen worden geverifieerd aan de hand van woordenlijsttermen nadat de LLM heeft gereageerd
- **Prototype pollution guard** — blokkeert `__proto__`, `constructor`, `prototype`
- **Padbeperking** — bestandsschrijfbewerkingen gevalideerd om binnen geconfigureerde directories te blijven
