...
Omdat de Coaching Data deel uitmaakt van het systeembericht, profiteert het van **prompt caching** — providers zoals Anthropic en Google cachen herhaalde systeemvoorvoegsels, zodat u slechts één keer per sessie betaalt voor de coachingcontext, niet één keer per batch.
...
Als een paar is geconfigureerd voor `llm-coached` maar er geen coachingbestand bestaat voor die locale, **valt rosetta terug op de standaard `llm` methode** met een waarschuwing in de console:
...
Dit betekent dat u `"defaultMethod": "llm-coached"` veilig globaal kunt instellen — talen met Coaching Data zullen deze gebruiken, en de rest krijgt standaard LLM-vertaling zonder fouten.
...
| Tier 2-talen (Koreaans, Turks, Thais) | `llm` met een register — LLM's verwerken deze adequaat met stijladvies |
| Tier 3-talen (Plains Cree, Yoruba, Quechua) | `llm-coached` — LLM's hebben grammaticaregels en woordenboeken nodig |
| Kunsttalen (Klingon, Sindarin, Kryptonian) | `llm-coached` — LLM's hebben enige trainingsdata maar hebben correcties nodig |
...
Richt u op **domeinspecifieke termen** die de LLM verkeerd zou doen of zou verzinnen. Besteed geen aandacht aan veelvoorkomende woorden die de LLM al aankan — richt u op de termen die specifiek zijn voor de UI van uw applicatie.
...
Dit geeft u chrF++, BLEU en exact match-scores. Maak meerdere versies van het coachingbestand aan en vergelijk deze — objectieve statistieken overtreffen subjectieve beoordelingen.
