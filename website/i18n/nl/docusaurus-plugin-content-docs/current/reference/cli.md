...
Interactieve installatiewizard die `i18n-rosetta.config.json` aanmaakt. Begeleidt u bij de bron-locale, doeltalen, het bestandsformaat en het vertaalmodel.
...
**Translation Memory**: Standaard laadt `sync` `.rosetta/tm.json` en levert het in de cache opgeslagen vertalingen voor ongewijzigde bronwaarden. Gebruik `--no-tm` om de cache te omzeilen (handig bij het wisselen van vertaalproviders of het debuggen van de kwaliteit). Zie [Translation Memory](/docs/concepts/translation-memory).
...
**Parallellisme**: Contentvertaling (Markdown, MDX, blogposts) draait in een platte work-item pool met configureerbare gelijktijdigheid. De standaard is 12 parallelle API-aanroepen. Overschrijf dit met `--concurrency` of het `concurrency` configuratieveld. JSON-sleutelvertaling verloopt sequentieel per locale (snel genoeg zodat parallellisme geen voordeel biedt).
...
Toon een lijst van alle onvertaalde fallback-waarden met het voorvoegsel `[EN]`. Sluit af met code 1 als er waarden worden gevonden — gebruik dit als een CI-gate om builds met onvolledige vertalingen te laten falen.
...
Scant de broncode op hardgecodeerde, gebruikersgerichte strings die i18n-vertaalaanroepen zouden moeten gebruiken. Detecteert automatisch uw framework (next-intl, react-i18next, vue-i18n, Hugo).
...
**Wat het detecteert:**
- Hardgecodeerde strings in JSX-tekst, `placeholder`, `alt`, `aria-label`, `title`
...
- Dekkingsscore — percentage van de strings dat via i18n verloopt
...
Plaatst hardgecodeerde strings die door `lint` zijn gedetecteerd automatisch in `t()` aanroepen. Maakt automatische back-ups voordat bestanden worden gewijzigd.
...
| Optie | Effect |
|--------|--------|
| `--locale <code>` | Wis alleen de items voor één locale |
...
| Subcommando | Uitvoer |
|------------|--------|
| `export` | Genereer `.xliff` uit bron- + doel-locale-bestanden |
...
| `--dry` | Bekijk een voorbeeld van de import zonder weg te schrijven |
...
**Automatische detectie:** De uitvoermap wordt afgeleid van uw projectstructuur:
...
|-------|---------|------|---------|
| **Lint** | `lint` | Pre-commit | Blokkeer commits met hardgecodeerde strings |
