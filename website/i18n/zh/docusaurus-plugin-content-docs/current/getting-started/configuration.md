...
:::note 尚未实现 typegen
配置加载器会识别并保留 `typegen` 配置块，但 TypeScript 类型生成功能尚未实现。这是为计划中的功能预留的占位符。设置这些值不会产生任何效果。
:::
...
| `batchSize` | `number` | `30` | 每个翻译批次的键数。数值越大 = API 调用越少，但提示词（prompt）越大。 |
...
每种语言都会从内置的语域（register）表中获取其默认语域。没有默认语域的语言将使用 `"Professional register."`。

### 包含语域字符串的对象

该值可以是语言卡片中的**预设键**，也可以是自定义语域文本：
...
| `name` | `string` | 易于人类阅读的语言名称（用于状态显示） |
| `model` | `string` | 覆盖默认模型 |
| `batchSize` | `number` | 覆盖默认批次大小 |
| `maxRetries` | `number` | 失败批次的最大重试预算（默认值：3） |
...
在项目根目录中创建 `.rosettaignore`，以将文件从 `lint` 扫描中排除。使用 glob 模式，类似于 `.gitignore`：
...
Rosetta 会在你的项目根目录中创建一个 `.rosetta/` 目录用于存放内部状态。通常你应该**将其添加到 `.gitignore`**——这是本地优化，而非项目源码：
...
你可以免费获得翻译、指导、重试循环、模型验证、质量层级和设置帮助等功能。只有 HTTP 请求的结构是特定于提供商的。对于使用原生 `fetch()` 的非 LLM 适配器，请使用 `lib/methods/fetch-with-retry.js` 中的共享 `fetchWithRetry()` 辅助函数，而无需编写自己的重试循环。
