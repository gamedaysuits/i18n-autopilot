...
Rosetta 支持零配置运行——它会自动从你的项目中检测本地化文件、格式和目标语言。如需更多控制权，请在项目根目录创建 `i18n-rosetta.config.json`，或运行：
...
| `localesDir` | `string` | `"./locales"` | 本地化文件路径。Rosetta 会扫描此目录。 |
...
| `defaultMethod` | `string` | `"llm"` | 默认翻译方法：`llm`、`llm-coached`、`google-translate`、`deepl`、`microsoft-translator`、`libretranslate`、`openai`、`anthropic`、`gemini`、`api`。会被 `--method` CLI 标志覆盖。 |
| `batchSize` | `number` | `30` | 每个翻译批次的键数。值越大 = API 调用越少，但提示词（prompts）更大。 |
| `fallbackPrefix` | `string` | `"[EN] "` | 添加到未翻译回退值的前缀。供 `audit` 用于检测不完整的翻译。 |
| `apiKeyEnvVar` | `string` | `"OPENROUTER_API_KEY"` | API 密钥的环境变量名称。可覆盖以使用自定义环境变量名。 |
...
Rosetta 会检查字符串是否与语言卡片中的预设键匹配。如果匹配，则使用卡片中完整的语域提示词。如果不匹配，则按原样使用该字符串。有关可用的预设，请参阅[支持的语言](/docs/reference/supported-languages#language-cards)。
...
在项目根目录创建 `.rosettaignore` 以从 `lint` 扫描中排除文件。使用 Glob 模式，类似于 `.gitignore`：
...
| `LLMCoachedMethod` | 辅导式 LLM (OpenRouter + 辅导数据) |
...
继承 `DirectLLMMethod`，只需约 40 行代码即可添加新的 LLM 提供商：
...
你可以免费获得翻译、辅导、重试循环、模型验证、质量层级和设置帮助等功能。只有 HTTP 请求格式是特定于提供商的。对于使用原生 `fetch()` 的非 LLM 适配器，请使用 `lib/methods/fetch-with-retry.js` 中的共享 `fetchWithRetry()` 辅助函数，而无需编写自己的重试循环。
...
- [同步工作原理](/docs/concepts/how-sync-works) — 翻译管道