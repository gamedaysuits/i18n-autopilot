...
| `batchSize` | `number` | `80` | 每个翻译批次的键数。数值越高 = API 调用越少，但提示词（prompts）越大。 |
| `jsonConcurrency` | `number` | `200` | JSON 键同步的最大并行语言环境翻译数。可被 `--json-concurrency` CLI 标志覆盖。 |
| `contentConcurrency` | `number` | `48` | 内容（Markdown/MDX）翻译的最大并行 API 调用数。可被 `--content-concurrency` CLI 标志覆盖。 |
| `fallbackPrefix` | `string` | `"[EN] "` | `audit` 和 `verify` 用于检测之前运行中遗留的未翻译值的标记前缀。Rosetta 不会写入此前缀——它仅读取该前缀用于检测。 |
| `apiKeyEnvVar` | `string` | `"OPENROUTER_API_KEY"` | API 密钥的环境变量名称。覆盖以使用自定义环境变量名称。 |
| `baseUrl` | `string` | `""` | SEO 产物生成（hreflang、sitemaps、JSON-LD）的基础 URL。 |
...
每种语言都会从内置语域（register）表中获取其默认语域。没有默认值的语言将使用 `"Professional register."`。
...
Rosetta 会在你的项目根目录中创建一个 `.rosetta/` 目录用于存放内部状态。通常你应该**将其添加到 `.gitignore` 中**——这是本地优化文件，而非项目源码：
...
| `backups/` | wrap 操作前的备份（由 `wrap --undo` 创建） | 否（安全网） |
...
你可以免费获得翻译、辅导、重试循环、模型验证、质量层级和设置帮助等功能。只有 HTTP 请求的结构是特定于提供商的。对于使用原生 `fetch()` 的非 LLM 适配器，请使用 `lib/methods/fetch-with-retry.js` 中共享的 `fetchWithRetry()` 辅助函数，而无需自己编写重试循环。
...
- [架构](/docs/concepts/architecture) —— 各组件如何连接
