...
:::note typegen 尚未实现
配置加载器会识别并保留 `typegen` 配置块，但 TypeScript 类型生成尚未实现。这是为计划中的功能预留的占位符。设置这些值不会产生任何效果。
:::
...
| `translatableFields` | `string[]` | `null` | 覆盖内容翻译时默认可翻译的 frontmatter 字段。`null` 使用内置默认值（`title`、`description`、`summary`）。 |
| `format` | `string` | `"auto"` | 文件格式：`json`、`toml`、`yaml` 或 `auto`（通过扩展名检测）。 |
...
| `batchSize` | `number` | `80` | 每个翻译批次的键数量。值越高 = API 调用越少，但提示词（prompt）越大。 |
| `jsonConcurrency` | `number` | `50` | JSON 键同步时的最大并行语言环境翻译数。可被 `--json-concurrency` CLI 标志覆盖。 |
| `contentConcurrency` | `number` | `12` | 内容（Markdown/MDX）翻译时的最大并行 API 调用数。可被 `--content-concurrency` CLI 标志覆盖。 |
| `fallbackPrefix` | `string` | `"[EN] "` | `audit` 和 `verify` 用于检测之前运行遗留的未翻译值的标记前缀。Rosetta 不会写入此前缀——它仅读取用于检测。 |
| `apiKeyEnvVar` | `string` | `"OPENROUTER_API_KEY"` | API 密钥的环境变量名称。用于覆盖自定义环境变量名称。 |
| `baseUrl` | `string` | `""` | SEO 产物生成（hreflang、站点地图、JSON-LD）的基础 URL。 |
| `pairs` | `object` | `{}` | 针对每个语言对的方法、模型和质量覆盖设置。请参阅[语言对配置](#pair-configuration)。 |
| `languages` | `object` | `{}` | 针对每种语言的覆盖设置。请参阅[语言配置](#language-configuration)。 |
...
| 字段 | 类型 | 描述 |
|-------|------|-------------|
| `register` | `string` | 风格/语气说明。可以是**预设键**（例如 `casual-tu`、`formal-hapsyo`）或自定义文本。请参阅[语言卡片](/docs/reference/supported-languages#language-cards)。 |
| `name` | `string` | 易于阅读的语言名称（用于状态显示） |
| `model` | `string` | 覆盖默认模型 |
...
Rosetta 会创建 `.i18n-rosetta.lock` 来跟踪已翻译源值的 SHA-256 哈希。**请提交此文件**，以便所有开发者共享相同的翻译基线。

当源值发生更改时，哈希将不再匹配，Rosetta 会在下次同步时重新翻译该键。
...
Rosetta 会在你的项目根目录中创建一个 `.rosetta/` 目录用于内部状态。通常你应该**将其添加到 `.gitignore`**——这是本地优化，而不是项目源码：
...
| `backups/` | Pre-wrap 备份（由 `wrap --undo` 创建） | 否（安全网） |

有关 `tm.json` 以及它如何节省 API 成本的详细信息，请参阅[翻译记忆库](/docs/concepts/translation-memory)。
...
| `loadCoachingData`、`findDictionaryMatches` | 辅导工具 |

### 自定义提供商扩展

继承 `DirectLLMMethod` 即可在约 40 行代码内添加一个新的 LLM 提供商：
...
你可以免费获得翻译、辅导、重试循环、模型验证、质量层级和设置帮助等功能。只有 HTTP 请求的结构是特定于提供商的。对于使用原生 `fetch()` 的非 LLM 适配器，请使用 `lib/methods/fetch-with-retry.js` 中共享的 `fetchWithRetry()` 辅助函数，而不是自己编写重试循环。
