...
3. **调整并发数** —— JSON 区域设置并行度默认为 200，内容并行度默认为 48。如果你的 API 提供商支持更高的速率限制：
   ...
   ```
4. **使用快速模型** —— `gpt-4o-mini` 比 `gpt-4o` 快得多
...
- **对第二梯队语言使用 Google Translate** —— 请参阅 [Translate 30 Languages](/docs/tutorials/translate-30-languages) 教程
...
有关缓存键设计的详细信息，请参阅 [Translation Memory](/docs/concepts/translation-memory)。

## 仍然遇到问题？

- **[GitHub Issues](https://github.com/gamedaysuits/i18n-rosetta/issues)** —— 搜索现有 Issue 或提交新 Issue
- **[Architecture Docs](/docs/concepts/architecture)** —— 了解系统设计
- **[Quality Gate](/docs/concepts/quality-gate)** —— 验证在底层的运作方式