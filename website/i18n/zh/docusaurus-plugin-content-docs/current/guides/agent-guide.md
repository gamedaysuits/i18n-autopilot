...
i18n-rosetta 是一个 CLI 工具，只需一条命令即可翻译你的应用本地化文件。本指南专为希望快速从零开始完成本地化文件翻译的 AI 智能体（或与 AI 智能体协作的开发者）编写。

:::tip 已经熟悉了？
如果你只需要命令，请跳转到 [CLI 参考](/docs/reference/cli)。如果你想构建并基准测试一种翻译方法，请参阅 [Arena 智能体指南](https://mtevalarena.org/docs/getting-started/agent-guide)。
:::
...
**API 密钥设置** — 根据你使用的方法，rosetta 至少需要一个密钥：
...
Rosetta 会自动检测你的本地化文件、文件格式（JSON、TOML、YAML、PO）以及你的目标语言：
...
2. 扫描你的源本地化文件，展平嵌套键
3. 与 `.i18n-rosetta.lock`（先前已翻译值的 SHA-256 哈希）进行比较
...
在更改一个键后的典型重新运行中，第 4 步会从缓存提供 142 个键，第 5 步仅翻译 1 个键。这就是为什么后续同步既快速又便宜的原因。

---

## 配置

在你的项目根目录中创建 `i18n-rosetta.config.json`：
...
| `batchSize` | 每次 API 调用的键数 | 30 (LLM), 128 (Google) |
| `concurrency` | 用于内容翻译的并行 API 调用数 | 12 |
...
| **`google-translate`** | GT 效果良好的高资源语言 | $20/百万字符 | `GOOGLE_TRANSLATE_API_KEY` |
...
对于 `llm-coached` 语言对，辅导数据通过明确的语言知识来引导 LLM。创建一个辅导文件：
...
质量门禁会验证词典术语是否确实出现在输出中——违规情况将被记录为 `[TERM]` 警告。
...
|-------|----------------|---------|
| **空/空白** | 模型未返回任何内容 | `""` |
| **源文本回显** | 模型原样返回了输入的英文 | 日语的 `"Welcome"` |
| **幻觉循环** | 重复的三元组 | `"Qo' Qo' Qo' Qo'"` |
| **长度膨胀** | 输出比源文本长 4 倍以上 | 10 字符源文本 → 50 字符输出 |
| **书写系统合规** | 本地化语言的书写系统错误 | 阿拉伯语本地化中出现拉丁文本 |

失败的检查会以 `[GATE]` 前缀记录。没有静默回退机制——如果翻译失败，它会被报告，而不是被悄悄接受。
...
Rosetta 将翻译缓存到 `.rosetta/tm.json` 中，以源文本 + 本地化语言 + 方法作为键。在后续同步中，未更改的键将从缓存中提供——无需 API 调用，没有成本。
...
| `.rosetta/coaching/` | 辅导数据目录 | **是** — 这是你的语言知识 |
...
npx i18n-rosetta sync
```
Rosetta 会按顺序处理语言对。借助 TM 缓存，只有更改的键才会调用 API。

**内容模式（适用于 Docusaurus、Hugo 等的 Markdown/MDX）：**
```bash
npx i18n-rosetta sync --content
```
与本地化 JSON 一起翻译文档、博客文章和内容文件。使用并行并发（默认：12 个同时进行的 API 调用）。
...
| `[GATE] Script compliance failed` | 你的目标本地化语言得到了拉丁文本而不是预期的书写系统——尝试使用其他模型或添加辅导数据 |
| `[GATE] Source echo` | 模型原样返回了英文——添加辅导数据或更换模型通常可以解决此问题 |
| 所有翻译均被缓存 | 使用 `--no-tm` 运行以绕过缓存，或使用 `--force-keys` 针对特定键运行 |
| 锁文件冲突 | `.i18n-rosetta.lock` 使用 SHA-256 哈希——通过保留任一版本来解决合并冲突是安全的，然后重新运行同步即可 |
...
- **想要构建你自己的翻译方法？** 请参阅 [Arena 智能体指南](https://mtevalarena.org/docs/getting-started/agent-guide) — 构建方法，证明其有效性，赢取奖品。