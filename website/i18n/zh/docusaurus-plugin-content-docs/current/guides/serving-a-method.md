...
description: "将复杂的翻译流水线（FST 门控、多步 LLM 链）封装为 HTTP 服务，并通过 api 方法将其接入 i18n-rosetta。"
---
# 将自定义方法作为 API 提供服务
...
1. **失败时返回空字符串** —— 不要将源字符串作为“翻译”返回。返回 `""`，i18n-rosetta 的质量网关会捕获它。该键将被跳过，并在下次同步时重试。
2. **包含置信度分数** —— 如果你的流水线可以评估质量，请在元数据中返回它。这有助于进行质量审计。
3. **实现健康检查** —— 添加一个 `GET /health` 端点，以便 i18n-rosetta 在开始大规模同步之前验证连接性。
4. **优雅地处理速率限制** —— 如果你的流水线有吞吐量限制，请返回 `429` 状态码。i18n-rosetta 的批处理系统将会自动退避。
5. **记录一切日志** —— 多步流水线可能会静默失败。记录每个步骤的输入/输出以便调试。
...
- [插件规范](/docs/reference/plugin-spec) —— `i18n-rosetta.config.json` 的完整架构，包括 `api` 方法字段
- [支持低资源语言](https://mtevalarena.org/docs/community/low-resource-languages) —— 资源不足语言的端到端指南，包括 OCAP 原则
- [架构](/docs/concepts/architecture) —— i18n-rosetta 的同步循环、批处理和方法分发是如何工作的
