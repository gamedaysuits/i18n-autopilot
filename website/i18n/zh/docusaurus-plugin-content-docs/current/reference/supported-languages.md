...
| 图标 | 方法 | 工作原理 | 成本 |
|------|--------|-------------|------|
| 🟢 | **Google Translate** | 神经机器翻译基准。支持 130+ 种语言。仅支持键值对字符串 — 无法安全地翻译 Markdown 内容。 | ~$20/100万字符 |
...
将 `GOOGLE_TRANSLATE_API_KEY` 设置为 Google Translate，或将 `OPENROUTER_API_KEY` 设置为 LLM 方法。详情请参阅 [翻译方法](/docs/guides/translation-methods)。
...
| 🇵🇭 | 菲律宾语 (Taglish) | `tl` / `fil` | ✅ | ✅ | ✅ | — | 在 Docusaurus 配置中使用 `fil`。rosetta 可解析两者。 |
...
商业机器翻译 (MT) 服务不支持这些语言。rosetta 为语言社区提供了在 [OCAP 原则](https://mtevalarena.org/docs/community/low-resource-languages) 下构建自有方法的工具。
...
:::info 平原克里语正在积极开发中
平原克里语的语域、辅导基础设施、书写系统转换器和评估测试工具均已可用，但翻译流水线**尚未发布**。我们正根据 [OCAP 原则](https://mtevalarena.org/docs/community/low-resource-languages) 与语言社区合作，以确保发布前的质量。请参阅 [支持低资源语言](https://mtevalarena.org/docs/community/low-resource-languages) 了解完整背景 — 以及你如何参与贡献。
:::

:::tip 添加更多低资源语言
rosetta 的方法插件系统正是为此设计的。语言社区可以构建自定义翻译方法，在自己的控制下进行托管，并通过 [API 方法](/docs/guides/serving-a-method) 提供服务。[方法排行榜](/leaderboard) 跟踪任何语言对的得分 — 构建一个方法，运行测试工具，并争取最高分。
:::
...
| 🏴‍☠️ | 海盗英语 | `x-pirate` | ❌ | ✅ | — | 仅限语域。航海隐喻。 |
| 🦸 | 氪星语 | `x-kryptonian` | ❌ | ✅ | 🔤 Latin→Kryptonian | 需要 PUA 字体。 |
| 🎭 | 莎士比亚英语 | `x-shakespeare` | ❌ | ✅ | — | 仅限语域。Thee/thou，-eth/-est 形式。 |
| 🐸 | 尤达语 | `x-yoda` | ❌ | ✅ | — | 仅限语域。OSV 语序。 |

有关 PUA 字体要求、Unicode 限制以及如何添加自定义语言，请参阅 [人造语言、书写系统与正字法](/docs/guides/conlangs-scripts-orthography)。
...
LLM 将利用其对该语言的训练知识进行翻译。设置 `register` 可以让你控制语气、正式程度和正字法约定。详情请参阅 [配置](/docs/getting-started/configuration)。
...
| 字段 | 包含内容 |
|-------|------------------|
| **`nativeName`** | 内名 — 该语言在自身书写系统中的自称 (例如：ქართული、Runasimi) |
...
- [配置](/docs/getting-started/configuration) — 完整的配置参考，包括语言设置
