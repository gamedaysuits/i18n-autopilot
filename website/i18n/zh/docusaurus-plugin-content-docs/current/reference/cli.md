...
交互式设置向导，用于创建 `i18n-rosetta.config.json`。引导你完成源语言环境、目标语言、文件格式和翻译模型的设置。
...
混合使用预设和独立代码：`european, ja` → fr, de, es, it, pt, nl, ja
...
**变更检测**：rosetta 在 `.i18n-rosetta.lock` 中存储 SHA-256 哈希值。当源值发生更改时，下一次 sync 会自动重新翻译这些键。请提交 lock 文件，以便所有开发者共享基线。

**并行处理**：内容翻译（Markdown、MDX、博客文章）在具有可配置并发数的扁平工作项池中运行。默认为 12 个并行 API 调用。可使用 `--concurrency` 或 `concurrency` 配置字段进行覆盖。JSON 键翻译按语言环境顺序运行（速度足够快，并行处理不会带来额外收益）。

**输出**：Sync 会显示版本横幅、格式/框架检测、成本估算以及每个语言环境的进度条：
...
进度条在每个批次（约 30 个键）后就地更新。使用 `--quiet` 仅显示错误/警告，或使用 `--json` 获取机器可读的 NDJSON 输出。两者都会隐藏进度条和横幅。
...
列出所有未翻译的带有 `[EN]` 前缀的回退值。如果发现任何未翻译项，则以代码 1 退出 —— 可用作 CI 门禁，在翻译不完整时使构建失败。
...
- 无效键 (Dead keys) —— 没有任何源文件引用的语言环境键
- 覆盖率得分 —— 通过 i18n 处理的字符串百分比

**排除项**：在你的项目根目录中创建 `.rosettaignore`（glob 模式，例如 `.gitignore`）。
...
为人造语言脚本转换器下载并管理 PUA Web 字体。使用私用区 (Private Use Area) 字符的语言（克林贡语、辛达林语、氪星语）需要自定义 Web 字体来渲染其脚本。此命令从经过验证的开源仓库下载它们。
...
将 `lint`、`sync` 和 `audit` 结合使用，实现万无一失的 i18n：
...
| 层级 | 命令 | 时机 | 目的 |
|-------|---------|------|---------|
| **Lint** | `lint` | Pre-commit | 拦截包含硬编码字符串的提交 |
...
- [质量门禁](/docs/concepts/quality-gate) — 如何验证翻译