...
辅导数据是 rosetta 用于向 LLM 教授其未受过训练的语言的机制。通过在每次翻译请求中提供语法规则、字典和样式说明，你可以将通用的 LLM 转化为适用于任何语言（包括目前完全没有 MT 支持的语言）的具备上下文感知能力的翻译器。
...
由于辅导数据是系统消息的一部分，它受益于**提示词缓存** —— 像 Anthropic 和 Google 这样的提供商会缓存重复的系统前缀，因此你每次会话只需为辅导上下文支付一次费用，而不是每批次支付一次。
...
| `dictionary` | `object` | 否 | 英文术语到目标语言术语的键值对映射。用于 LLM 可能不知道的特定领域词汇。 |
| `style_notes` | `string` | 否 | 自由格式的样式说明（语域、语气、正式程度约定）。 |

所有字段都是可选的 —— 你可以从仅包含字典开始，并在不断完善的过程中添加语法规则。
...
| 第二梯队语言（韩语、土耳其语、泰语） | 带有语域设置的 `llm` —— LLM 在样式指导下能很好地处理这些语言 |
...
将规则写成**指令**，而不是描述。LLM 遵循指令的效果比它理解语言学理论的效果更好。
...
- [插件规范](/docs/reference/plugin-spec) —— 将辅导数据打包到插件中
