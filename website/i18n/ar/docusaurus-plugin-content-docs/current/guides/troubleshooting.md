...
- **محركات أقراص الشبكة** — لا يعمل `fs.watch` بشكل موثوق على حوامل NFS/SMB
...
1. **استخدام Google Translate للأزواج اللغوية ذات الحجم الكبير** — إنه أسرع بـ 10-50 مرة من الترجمة باستخدام النماذج اللغوية الكبيرة (LLM)
2. **زيادة حجم الدفعة** (الافتراضي هو 80):
   ```json
   { "batchSize": 120 }
   ```
3. **ضبط التزامن (concurrency)** — التوازي الافتراضي لملفات JSON هو 200 وللمحتوى 48. إذا كان مزود API الخاص بك يدعم حدود معدل طلبات أعلى:
   ```bash
...
- **استخدام ذاكرة الترجمة (Translation Memory)** — يتم تشغيلها افتراضيًا. قم بتشغيل `i18n-rosetta tm stats` للتحقق من عملها. إذا رأيت 0 إدخالات بعد مزامنات متعددة، فقد يكون هناك خطأ ما في أذونات دليل `.rosetta/` الخاص بك
- **استخدام التخزين المؤقت للمطالبات (prompt caching)** — يقوم Rosetta بتقسيم رسائل النظام/المستخدم لتحقيق إصابات التخزين المؤقت (cache hits) على نماذج Anthropic وGoogle
...
- **[وثائق البنية (Architecture Docs)](/docs/concepts/architecture)** — فهم تصميم النظام
- **[بوابة الجودة (Quality Gate)](/docs/concepts/quality-gate)** — كيف تعمل عملية التحقق خلف الكواليس