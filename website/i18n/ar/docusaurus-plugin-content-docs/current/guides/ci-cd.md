...
قم بتشغيل الترجمات وفقاً لجدول زمني مع إجراء التزام تلقائي:
...
إذا كنت تستخدم نقطة نهاية (endpoint) للترجمة عن بُعد (على سبيل المثال، خدمة ترجمة مستضافة):
...
للحصول على أقصى تغطية لـ i18n، قم بتأمين مسار عملك باستخدام الأدوات الثلاث جميعها:
...
|-------|---------|------|---------|
| **Lint** | `lint` | Pre-commit | حظر الالتزامات التي تحتوي على نصوص ثابتة (hardcoded) |
| **Sync** | `sync` | Post-commit / CI | ترجمة المفاتيح المفقودة والمعدلة |
| **Audit** | `audit` | خطوة البناء | إفشال النشر إذا كانت أي لغة (locale) غير مكتملة |
...
- [بوابة الجودة (Quality Gate)](/docs/concepts/quality-gate) — ماذا يحدث عندما تفشل الترجمات
