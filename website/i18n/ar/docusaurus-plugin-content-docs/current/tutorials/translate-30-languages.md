...
sidebar_position: 2
title: "ترجمة 30 لغة"
description: "دليل إرشادي: توسيع نطاق مشروع من 3 لغات إلى 30 لغة باستخدام دمج الأساليب لكل زوج لغوي، والمعالجة المجمعة، والتكامل مع CI."
---
# دليل عملي: ترجمة 30 لغة

قم بتوسيع نطاق مشروعك من عدد قليل من اللغات المحلية إلى تغطية عالمية. يستعرض هذا الدليل العملي كيفية اختيار الطريقة، وتحسين التكلفة، والتكامل مع التكامل المستمر (CI) لعملية نشر حقيقية متعددة اللغات.
...
| **المستوى 3 — موجه** | `crk`, `oj`, `mi`, `haw` | `llm-coached` + plugins | موارد محدودة، تتطلب فرض المصطلحات |
...
**ملاحظة:** اللغات غير المدرجة في `pairs` ترث `defaultMethod: "google-translate"`. لا تحتاج إلى سرد جميع اللغات الثلاثين.

:::info
دعم `crk` قيد التطوير — راجع [دعم لغة ذات موارد محدودة](https://mtevalarena.org/docs/community/low-resource-languages) لمعرفة الحالة وإرشادات المساهمة.
:::

## الخطوة 3: إعداد مفاتيح API

ستحتاج إلى كلا مفتاحي API لهذا التكوين:
...
- كم عدد المفاتيح الجديدة/المعدلة لكل لغة محلية
- استدعاءات API المقدرة لكل مستوى
...
يعالج Rosetta كل زوج لغوي بشكل مستقل. ستكون أزواج المستوى 2 التي تستخدم Google Translate سريعة. ستكون أزواج المستوى 1 التي تستخدم LLM أبطأ ولكن بجودة أعلى. تستخدم أزواج المستوى 3 الموجهة بيانات التوجيه الخاصة بالإضافة (plugin).
...
يُخرج هذا جدولاً يوضح طريقة كل زوج، والنموذج، ومستوى الجودة، وما إذا كانت بيانات التوجيه أو درجات التقييم (benchmark scores) متاحة.
...
تكلف عمليات المزامنة التدريجية (5–20 مفتاحًا معدلاً) جزءًا بسيطًا من تكلفة المزامنة الكاملة.
...
- [دعم لغة ذات موارد محدودة](https://mtevalarena.org/docs/community/low-resource-languages) — إضافة بيانات توجيه للغات التي لا تحظى بتغطية واسعة في الترجمة الآلية (MT)