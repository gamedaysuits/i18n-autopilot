...
i18n-rosetta는 **플러그 가능한 메서드 시스템(pluggable method system)**을 사용해요. 각 언어 쌍은 서로 다른 번역 메서드(LLM, coached, script-converter 등)를 사용할 수 있어요. 메서드는 `lib/translate.js`에 등록되고 `lib/pairs.js`을 통해 언어 쌍별로 해석돼요.

eval harness의 역할은 번역 메서드를 **개발, 테스트 및 내보내기**하는 것이에요. i18n-rosetta의 역할은 이를 **소비하고 실행**하는 것이죠. harness는 절대 rosetta 내부에서 실행되지 않아요.
...
메서드 플러그인은 단일 JSON 파일(`method.json`)과 선택적인 코칭 데이터 파일들로 구성돼요.
...
| `type` | string | ✅ | Rosetta 메서드 타입: `llm`, `llm-coached`, `api`, `google-translate`, `deepl`, `microsoft-translator`, `libretranslate`, `openai`, `anthropic`, `gemini` |
| `version` | string | ✅ | Semver 버전 (예: `1.0.0`) |
| `locales` | string[] | ✅ | 이 메서드가 대상으로 하는 로케일 코드 (최소 1개) |
...
| `config.batchSize` | number | — | API 배치당 키 개수 (1–200, 기본값: 30) |
| `config.temperature` | number | — | LLM temperature (0.0–2.0, 기본값: 0.3) |
| `benchmarks` | object | — | 로케일별 벤치마크 결과 |
...
| `exact_match_rate` | number | ✅ | 0.0–1.0, 정확히 일치하는 비율 (exact match) |
...
:::info 어떤 지표가 표시되나요?
`rosetta status` 명령어는 벤치마크 블록의 **chrF++** 및 **정확도(exact match rate)**를 표시해요. `corpus_bleu`는 매니페스트에서 허용되지만 현재 어떤 rosetta 명령어에서도 표시되거나 사용되지 않아요. [메서드 리더보드](/leaderboard)는 chrF++, 정확도, 그리고 FST 승인율(acceptance rate)을 추적해요.
:::
...
출처(provenance) 블록은 플러그인에 번들로 제공되는 리소스의 라이선스 상태를 전달해요.

| 필드 | 타입 | 기본값 | 설명 |
|-------|------|---------|-------------|
| `resources` | object[] | `[]` | `name`, `license`, `type`가 포함된 번들 리소스 목록 |
...
| 필드 | 타입 | 필수 여부 | 설명 |
|-------|------|----------|-------------|
| `grammar_rules` | string[] | — | 이 로케일의 모든 LLM 프롬프트에 주입되는 규칙 |
...
플러그인은 *어떤* 메서드를 사용할지 정의해요(`type`). 언어 쌍 구성(pair config)은 이를 *어떻게* 실행할지 조정해요(`model`, `register`, `batchSize`). 언어 쌍에서 `model`을 설정하면, 플러그인의 기본값을 덮어써요.
:::
...
3. 플러그인의 `coaching/` 디렉터리에서 코칭 데이터를 불러와요.
4. `config` 블록을 사용하여 모델/어조/temperature의 빈 공간을 채워요.
5. `benchmarks` 블록은 `rosetta status` 출력에 표시돼요.
6. `rosetta provenance`가 `provenance` 블록의 라이선스 플래그를 확인해요.

---

## 스키마 유효성 검사

플러그인 매니페스트는 설치 시 [`schemas/rosetta-plugin.schema.json`](https://github.com/gamedaysuits/i18n-rosetta/blob/main/schemas/rosetta-plugin.schema.json)을 기준으로 유효성 검사가 진행돼요.
...
- ❌ Python 코드나 harness 종속성
- ❌ 원시 말뭉치(corpus) 데이터나 실행 로그
- ❌ API 키나 자격 증명
- ❌ harness 구성
- ❌ 내부 프롬프트 템플릿 (이는 rosetta의 메서드 구현 내에 존재해요)
...
- [구성](/docs/getting-started/configuration) — 언어 쌍 및 언어별 구성
- [API를 통한 메서드 제공](/docs/guides/serving-a-method) — 메서드를 HTTP 서비스로 호스팅하기
