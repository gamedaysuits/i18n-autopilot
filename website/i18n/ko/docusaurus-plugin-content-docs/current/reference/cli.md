...
특정 명령어에 대한 자세한 도움말을 보려면 `i18n-rosetta <command> --help`을(를) 실행하세요.
...
`i18n-rosetta.config.json`을(를) 생성하는 대화형 설정 마법사예요. 소스 로케일, 대상 언어, 파일 형식 및 번역 모델 설정을 안내해요.
...
**`--langs` 옵션**: 쉼표로 구분된 대상 언어 코드 목록이에요. 언어 선택 프롬프트를 건너뛰고 각 언어에 대한 기본 레지스터 프리셋을 적용해요. 완전히 비대화형으로 설정하려면 `--yes`와(과) 함께 사용하세요.

**언어 프리셋**: 대상 언어를 묻는 프롬프트가 나타나면 프리셋 이름을 입력할 수 있어요:
...
모든 로케일 파일에서 누락되거나 오래된 키, 폴백(fallback) 키를 번역해요.
...
**Translation Memory**: 기본적으로 `sync`은(는) `.rosetta/tm.json`을(를) 로드하고 변경되지 않은 소스 값에 대해 캐시된 번역을 제공해요. 캐시를 우회하려면 `--no-tm`을(를) 사용하세요(번역 제공자를 변경하거나 품질을 디버깅할 때 유용해요). [Translation Memory](/docs/concepts/translation-memory)를 참조하세요.

**변경 감지**: rosetta는 `.i18n-rosetta.lock`에 SHA-256 해시를 저장해요. 소스 값이 변경되면 다음 동기화 시 해당 키를 자동으로 다시 번역해요. 모든 개발자가 기준선을 공유할 수 있도록 잠금 파일(lock file)을 커밋하세요.

**병렬 처리**: 콘텐츠 번역(Markdown, MDX, 블로그 게시물)은 구성 가능한 동시성을 가진 평면적인 작업 항목 풀에서 실행돼요. 기본값은 12개의 병렬 API 호출이에요. `--concurrency` 또는 `concurrency` 구성 필드로 재정의할 수 있어요. JSON 키 번역은 로케일별로 순차적으로 실행돼요(충분히 빠르기 때문에 병렬 처리가 이점을 주지 않아요).

**출력**: 동기화 시 버전 배너, 형식/프레임워크 감지, 예상 비용 및 로케일별 진행률 표시줄이 표시돼요:
...
진행률 표시줄은 각 배치(약 30개 키) 후에 제자리에서 업데이트돼요. 오류/경고만 보려면 `--quiet`을(를) 사용하고, 기계가 읽을 수 있는 NDJSON 출력을 원하면 `--json`을(를) 사용하세요. 두 옵션 모두 진행률 표시줄과 배너를 숨겨요.

---

## watch

소스 로케일 파일이 변경될 때 자동으로 동기화해요. `Ctrl+C`(으)로 중단할 때까지 실행돼요.
...
번역되지 않은 `[EN]` 접두사가 붙은 모든 폴백 값을 나열해요. 발견되는 항목이 있으면 종료 코드 1로 종료돼요. 불완전한 번역이 있을 때 빌드를 실패하게 만드는 CI 게이트로 활용하세요.
...
i18n 번역 호출을 사용해야 하는 하드코딩된 사용자 대면 문자열이 있는지 소스 코드를 스캔해요. 프레임워크(next-intl, react-i18next, vue-i18n, Hugo)를 자동으로 감지해요.
...
- 사용자 대면 콘텐츠가 있지만 i18n 프레임워크 임포트가 없는 파일
- 데드 키 — 어떤 소스 파일에서도 참조하지 않는 로케일 키
- 커버리지 점수 — i18n을 거치는 문자열의 비율

**제외 대상**: 프로젝트 루트에 `.rosettaignore`을(를) 생성하세요(`.gitignore`와(과) 같은 glob 패턴).

---

## wrap

`lint`(이)가 감지한 하드코딩된 문자열을 `t()` 호출로 자동 래핑해요. 파일을 수정하기 전에 자동으로 백업을 생성해요.
...
**확인 항목:**
- 자리 표시자 손상 (예: 소스에는 `{name}`이(가) 있지만 대상에는 누락됨)
- 인코딩 문제 (글자 깨짐(mojibake), 잘못된 유니코드)
- 번역되지 않은 복사본 (대상 값이 소스와 동일함)
- 고립된 키 (대상에는 있지만 소스에는 없는 키)
...
Translation Memory 캐시(`.rosetta/tm.json`)를 관리해요. TM은 이전 번역을 저장하고, 후속 동기화 시 API를 호출하는 대신 저장된 번역을 제공해요.
...
TM의 작동 방식과 지워야 할 시기에 대해서는 [Translation Memory](/docs/concepts/translation-memory)를 참조하세요.
...
전체 워크플로우는 [Working with Professional Translators](/docs/guides/professional-translators)를 참조하세요.

---

## status

페어 구성, 설치된 플러그인, 품질 계층 및 벤치마크 점수를 표시해요.
...
번역 메서드 플러그인을 관리해요. 플러그인은 `.rosetta/methods/`에 설치되는 사전 패키징된 번역 레시피예요.
...
플러그인 매니페스트 형식은 [Plugin Specification](/docs/reference/plugin-spec)을 참조하세요.

---

## fonts

인공어 스크립트 변환기를 위한 PUA 웹 폰트를 다운로드하고 관리해요. 사용자 정의 영역(Private Use Area) 문자를 사용하는 언어(클링온어, 신다린어, 크립톤어)는 스크립트를 렌더링하기 위해 맞춤형 웹 폰트가 필요해요. 이 명령어는 검증된 오픈 소스 저장소에서 해당 폰트를 다운로드해요.
...
**네이티브 유니코드 변환기**(`crk` → 크리어 음절 문자, `sr` → 세르비아어 키릴 문자)는 폰트 설치가 필요하지 않아요.

PUA 폰트에 대한 자세한 내용은 [Conlangs, Scripts & Orthography](/docs/guides/conlangs-scripts-orthography)를 참조하세요.

## 3계층 파이프라인

완벽한 i18n을 위해 `lint`, `sync` 및 `audit`을(를) 함께 사용하세요:
...
- [Translation Methods](/docs/guides/translation-methods) — 페어별 메서드 선택
...
- [How Sync Works](/docs/concepts/how-sync-works) — 동기화 파이프라인 이해
