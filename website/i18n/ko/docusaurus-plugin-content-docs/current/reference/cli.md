...
`i18n-rosetta.config.json` 파일을 생성하는 대화형 설정 마법사입니다. 소스 로케일, 대상 언어, 파일 형식 및 번역 모델 설정을 안내해요.
...
**`--langs` 옵션**: 쉼표로 구분된 대상 언어 코드 목록입니다. 언어 선택 프롬프트를 건너뛰고 각 언어에 대한 기본 어조(register) 프리셋을 적용해요. 완전히 비대화형으로 설정하려면 `--yes`와 결합하세요.
...
**번역 메모리(Translation Memory)**: 기본적으로 `sync`은 `.rosetta/tm.json`를 로드하고 변경되지 않은 소스 값에 대해 캐시된 번역을 제공해요. 캐시를 우회하려면 `--no-tm`을 사용하세요(번역 제공자를 변경하거나 품질을 디버깅할 때 유용해요). [번역 메모리](/docs/concepts/translation-memory)를 참조하세요.

**변경 감지**: rosetta는 `.i18n-rosetta.lock`에 SHA-256 해시를 저장해요. 소스 값이 변경되면 다음 동기화 시 해당 키를 자동으로 다시 번역해요. 모든 개발자가 기준을 공유할 수 있도록 잠금 파일(lock file)을 커밋하세요.

**병렬 처리**: JSON 키 번역과 콘텐츠 번역 모두 병렬로 실행돼요. JSON 로케일은 동시에 번역되며(기본값: 50개 로케일 동시 처리), 각 로케일 내의 배치(batch)도 병렬화돼요(4개 배치 동시 처리). 콘텐츠 번역(Markdown, MDX, 블로그 게시물)은 플랫 작업 항목 풀에서 실행돼요(기본값: 12개 API 호출 동시 처리). `--json-concurrency`, `--content-concurrency` 또는 `--concurrency`(둘 다 설정)를 사용하여 재정의할 수 있어요.
...
진행률 표시줄은 각 배치(약 80개 키) 후에 제자리에서 업데이트돼요. 오류/경고만 보려면 `--quiet`를 사용하고, 기계가 읽을 수 있는 NDJSON 출력을 원하면 `--json`을 사용하세요. 두 옵션 모두 진행률 표시줄과 배너를 숨겨요.
...
이전 실행에서 번역되지 않은 `[EN]` 접두사가 붙은 폴백(fallback) 값을 모두 나열해요. 발견된 항목이 있으면 종료 코드 1로 종료돼요. 불완전한 번역이 있을 때 빌드를 실패하게 만드는 CI 게이트로 활용하세요.
...
디스크에서 모든 로케일 파일을 다시 읽고 번역이 실제로 존재하며 올바른지 검증해요. 이는 (`--no-verify`이 전달되지 않는 한) 모든 `sync`의 끝에 자동으로 실행되는 것과 동일한 검증이에요.
...
**검증 항목:**
- 키 패리티(Key parity) — 모든 소스 키가 각 대상에 존재하는지 확인
- 이전 실행의 `[EN]` 폴백 마커
- 비어 있는 번역
...
- 사용자 대면 콘텐츠가 있지만 i18n 프레임워크 가져오기(import)가 없는 파일
- 데드 키(Dead keys) — 어떤 소스 파일에서도 참조하지 않는 로케일 키
- 커버리지 점수 — i18n을 거치는 문자열의 비율

**제외**: 프로젝트 루트에 `.rosettaignore` 파일을 생성하세요(`.gitignore`과 같은 glob 패턴 사용).

---

## wrap

`lint`로 감지된 하드코딩된 문자열을 `t()` 호출로 자동 래핑해요. 파일을 수정하기 전에 자동으로 백업을 생성해요.
...
**검증 항목:**
- 자리 표시자 손상 (예: 소스에는 `{name}`이 있지만 대상에는 누락됨)
- 인코딩 문제 (글자 깨짐(mojibake), 잘못된 Unicode)
...
번역 메모리(Translation Memory) 캐시(`.rosetta/tm.json`)를 관리해요. TM은 이전 번역을 저장하고, 이후 동기화 시 API를 호출하는 대신 캐시된 번역을 제공해요.
...
TM의 작동 방식과 삭제 시기에 대해서는 [번역 메모리](/docs/concepts/translation-memory)를 참조하세요.
...
전체 워크플로우는 [전문 번역가와 협업하기](/docs/guides/professional-translators)를 참조하세요.

---

## status

쌍(pair) 구성, 설치된 플러그인, 품질 등급 및 벤치마크 점수를 표시해요.
...
인공어(constructed language) 스크립트 변환기를 위한 PUA 웹 폰트를 다운로드하고 관리해요. 사용자 정의 영역(Private Use Area) 문자를 사용하는 언어(Klingon, Sindarin, Kryptonian)는 스크립트를 렌더링하기 위해 사용자 지정 웹 폰트가 필요해요. 이 명령어는 검증된 오픈 소스 저장소에서 해당 폰트를 다운로드해요.
...
PUA 폰트에 대한 자세한 내용은 [인공어, 스크립트 및 정서법](/docs/guides/conlangs-scripts-orthography)을 참조하세요.

## 3계층 파이프라인

완벽한 i18n을 위해 `lint`, `sync` 및 `audit`를 함께 사용하세요:
...
| **Audit** | `audit` | 빌드 단계 | 로케일에 `[EN]` 마커가 있는 경우 배포 실패 처리 |

---

## 참고 항목

- [구성](/docs/getting-started/configuration) — 구성 파일 참조
- [번역 방식](/docs/guides/translation-methods) — 쌍(pair)별 번역 방식 선택
