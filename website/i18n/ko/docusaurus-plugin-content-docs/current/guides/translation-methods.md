...
Rosetta는 10가지 번역 방식을 지원해요. 각 언어 쌍마다 다른 방식을 사용할 수 있으며, 전체 프로젝트에 대해 하나의 방식만 고집할 필요가 없어요.
...
속도와 비용 중심. 대용량 키-값(key-value) 쌍에 가장 적합해요.
...
**사용 시기:** 자원이 부족한 언어(low-resource languages), 도메인 특화 용어(법률, 의료), 격식 있는 어조가 필요하거나 일반적인 LLM 출력이 충분히 정확하지 않은 모든 경우에 사용해요.
...
직접 LLM 제공업체(`openai`, `anthropic`, `gemini`)는 처음 사용할 때 모델 문자열을 검증해요. 이를 통해 세 가지 유형의 실수를 잡아낼 수 있어요.
...
**사용 시기:** 뉘앙스보다 속도와 비용이 더 중요한 대용량 키-값 문자열 쌍에 적합해요. 기본적으로 130개 이상의 언어를 지원해요.

**제한 사항:**
- ⚠️ **Markdown 인식 불가.** 코드 블록, 숏코드, 보간(interpolation) 변수가 손상될 수 있어요.
- 어조/말투 제어 불가
- 코칭 또는 용어 강제 적용 불가
...
:::tip 자동 감지
`GOOGLE_TRANSLATE_API_KEY`만 설정되어 있고(OpenRouter 키 없음) 구성 변경이 없다면, rosetta는 자동으로 Google Translate로 전환해요.
:::

## `deepl` — DeepL API

DeepL 번역 API와 직접 연동돼요. 일관된 용어 사용을 위해 용어집(glossary)을 지원해요.
...
**사용 시기:** 오프라인 번역, 데이터 개인정보 보호 규정(GDPR) 준수 또는 무비용 운영이 필요한 프로젝트에 적합해요. 특히 외부 API에 의존해서는 안 되는 CI 파이프라인에 매우 유용해요.
...
커뮤니티 호스팅 또는 IP로 보호되는 번역 엔드포인트를 위한 씬(thin) HTTP 클라이언트예요. Rosetta는 키를 전송하고 번역 결과를 돌려받기만 하며, 자체적인 번역 로직은 전혀 포함되어 있지 않아요.

**사용 시기:** 번역 방식이 서버 측에 호스팅된 경우(예: 배포할 수 없는 독점 코칭 데이터, 파인튜닝된 모델, FST 파이프라인 등)에 사용해요.
...
`api` 방식은 **OCAP 호환 커뮤니티 호스팅 번역**으로 연결되는 다리 역할을 해요. 원주민 및 소수 언어 커뮤니티는 자체 번역 엔드포인트를 호스팅하여 코칭 데이터, 파인튜닝된 모델, 언어적 IP를 커뮤니티의 통제 하에 둘 수 있으며, Rosetta는 씬 클라이언트로 이에 연결돼요.
...
이 설정은 프랑스어는 DeepL(용어집 지원), 일본어는 OpenAI(품질), 한국어는 Gemini(무료 티어), 아랍어는 Microsoft Translator(지원 범위), 평원 크리어(Plains Cree)는 코칭된 플러그인(특화)을 통해 번역해요.

## 플러그인

플러그인은 특정 언어 쌍을 위해 미리 패키징된 번역 레시피예요. 코드가 아닌 JSON 매니페스트로, rosetta에게 어떤 방식을 어떤 설정으로 사용할지, 그리고 어떤 품질로 벤치마크되었는지 알려줘요.

:::tip 단일 명령어로 평가 하네스에서 프로덕션까지
...
방식을 변경하시나요? 모델 형식과 환경 변수가 달라지므로 다음 매핑을 참고하세요.
...
- 각 직접 제공업체는 고유한 환경 변수를 가져요(`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`).
...
환경 변수를 변경하지 않고 모델을 전환하고 싶거나, 단일 키로 200개 이상의 모델에 접근하고 싶을 때는 **OpenRouter를 사용하세요**. 청구를 간소화하거나, 지연 시간을 줄이거나(중개자 없음), Anthropic의 프롬프트 캐싱과 같은 제공업체별 기능에 접근하고 싶을 때는 **직접 제공업체를 사용하세요**.
:::
...
| `deepl` | ~$0.02 | 빠름 | 좋음 | 유럽 언어, 전문 용어 |
| `microsoft-translator` | ~$0.01 | 빠름 | 적절함 | Azure 환경, 넓은 언어 지원 범위 |
| `libretranslate` | **무료** (셀프 호스팅) | 다양함 | 보통 | 에어갭(Air-gapped), GDPR, CI 파이프라인 |
| `gemini` (Pro) | ~$0.07 | 보통 | 매우 좋음 | 품질 민감, 무료 할당량 |
| `openai` (GPT-4o-mini) | ~$0.01 | 빠름 | 좋음 | 예산 절감형 LLM |
| `openai` (GPT-4o) | ~$0.10 | 보통 | 매우 좋음 | 품질 민감 |
| `anthropic` (Haiku) | ~$0.01 | 빠름 | 좋음 | 예산 절감형 LLM |
| `anthropic` (Sonnet) | ~$0.10 | 보통 | 매우 좋음 | 품질 민감 |
| `anthropic` (Opus) | ~$0.50 | 느림 | 훌륭함 | 최고 품질 |
