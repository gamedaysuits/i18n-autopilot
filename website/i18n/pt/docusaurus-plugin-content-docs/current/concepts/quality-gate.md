...
Toda tradução passa por uma etapa de validação determinística antes de ser gravada em disco. O Quality Gate captura falhas comuns de tradução automática — sem fallbacks silenciosos, sem lixo gravado em seus arquivos de localidade.
...
Analisa padrões de trigramas (3 caracteres) na saída. Se algum trigrama se repetir mais do que um número limite de vezes em relação ao tamanho da saída, a tradução será rejeitada. Isso captura saídas degeneradas como `"Qo' Qo' Qo' Qo' Qo'"`.

### Inflação de Tamanho

Rejeita traduções em que o tamanho da saída excede `maxLengthRatio × source length` (padrão: 4×). Isso captura alucinações do modelo que produzem paredes de texto para uma entrada curta.

Configurável via `maxLengthRatio` na sua configuração.

### Conformidade de Script

Para localidades com um campo `script` configurado (ex., `"script": "cans"` para Plains Cree Syllabics), valida se a saída contém caracteres não ASCII apropriados para o script de destino. Saídas apenas em latim para uma localidade em árabe, CJK ou silábica são rejeitadas.
...
Após esgotar as novas tentativas, as chaves com problemas são registradas e ignoradas. Elas serão tentadas novamente na próxima execução do `sync`.
...
## Validação ICU MessageFormat

O comando `integrity` valida padrões de plural do ICU MessageFormat em relação às regras de plural do CLDR. Se o seu arquivo de origem usar a sintaxe ICU como:
...
Para pares treinados com um dicionário, o rosetta executa uma verificação de terminologia pós-tradução. Após a aprovação no Quality Gate, ele verifica se o LLM realmente usou os termos exigidos do dicionário.
...
Violações de terminologia são **avisos, não erros de bloqueio**. A tradução ainda é gravada em disco. Isso é intencional — o LLM pode ter motivos válidos para escolher uma alternativa (contexto, gramática), e bloquear por incompatibilidade de termos causaria mais danos do que benefícios.
...
- [Dados de Treinamento](/docs/concepts/coaching-data) — melhorando a qualidade da tradução upstream
