...
:::tip Já conhece?
Se você precisa apenas dos comandos, pule para a [Referência da CLI](/docs/reference/cli). Se você deseja criar e avaliar um método de tradução, consulte o [Guia do Agente da Arena](https://mtevalarena.org/docs/getting-started/agent-guide).
:::
...
O Rosetta lê `.env` automaticamente. Obtenha uma chave do OpenRouter em [openrouter.ai/keys](https://openrouter.ai/keys).
...
2. Verifica seu arquivo de localidade de origem e nivela as chaves aninhadas (flatten)
...
Em uma nova execução típica após alterar uma chave, a etapa 4 fornece 142 chaves do cache e a etapa 5 traduz 1 chave. É por isso que as sincronizações subsequentes são rápidas e baratas.
...
| `pairs` | Mapa de origem→destino com configuração do método | (obrigatório) |
...
| `batchSize` | Chaves por chamada de API | 30 (LLM), 128 (Google) |
| `concurrency` | Chamadas de API paralelas para tradução de conteúdo | 12 |
...
| **Eco da origem** | O modelo retornou a entrada em inglês inalterada | `"Welcome"` para o japonês |
| **Loop de alucinação** | Trigramas repetidos | `"Qo' Qo' Qo' Qo'"` |
| **Inflação de tamanho** | A saída é 4x+ maior que a origem | Origem com 10 caracteres → Saída com 50 caracteres |
| **Conformidade de script** | Script errado para a localidade | Texto latino para localidade em árabe |

As falhas são registradas com o prefixo `[GATE]`. Sem fallbacks silenciosos — se uma tradução falhar, ela será relatada, e não aceita silenciosamente.
...
O Rosetta armazena as traduções em cache no `.rosetta/tm.json`, indexadas por texto de origem + localidade + método. Nas sincronizações subsequentes, as chaves inalteradas são fornecidas pelo cache — sem chamadas de API, sem custos.
...
O Rosetta cria vários arquivos no seu projeto. Saiba o que eles são para não excluir ou commitar os arquivos errados acidentalmente:

| Arquivo | Propósito | Git? |
|------|---------|------|
| `.i18n-rosetta.lock` | Hashes SHA-256 dos valores de origem traduzidos (detecção de alterações) | **Sim** — faça o commit disso |
| `.i18n-rosetta-content.lock` | O mesmo, mas para arquivos de conteúdo Markdown/MDX | **Sim** — faça o commit disso |
| `.rosetta/tm.json` | Cache da Memória de Tradução | **Sim** — faça o commit disso (economiza custos de API para a equipe) |
| `.rosetta/coaching/` | Diretório de dados de coaching | **Sim** — este é o seu conhecimento linguístico |
| `i18n-rosetta.config.json` | Configuração do projeto | **Sim** — faça o commit disso |
...
npx i18n-rosetta sync
```
O Rosetta processa os pares sequencialmente. Com o cache da TM, apenas as chaves alteradas atingem a API.

**Modo de conteúdo (Markdown/MDX para Docusaurus, Hugo, etc.):**
```bash
npx i18n-rosetta sync --content
```
Traduz documentos, postagens de blog e arquivos de conteúdo junto com o JSON de localidade. Usa concorrência paralela (padrão: 12 chamadas de API simultâneas).

**Execução de teste (dry run - visualização sem gravar):**
```bash
...
**Auditoria de fallbacks não traduzidos:**
```bash
...
| `[GATE] Script compliance failed` | Sua localidade de destino recebeu texto latino em vez do script esperado — tente um modelo diferente ou adicione dados de coaching |
