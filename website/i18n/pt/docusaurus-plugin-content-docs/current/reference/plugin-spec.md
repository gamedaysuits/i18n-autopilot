...
| `locales` | string[] | ✅ | Quais códigos de localidade este método atende (mínimo 1) |
...
| `config.batchSize` | number | — | Chaves por lote de API (1–200, padrão: 30) |
...
O comando `rosetta status` exibe **chrF++** e a **taxa de correspondência exata** (exact match rate) do bloco de benchmark. `corpus_bleu` é aceito no manifesto, mas atualmente não é exibido ou usado por nenhum comando do rosetta. O [Method Leaderboard](/leaderboard) rastreia chrF++, correspondência exata e taxa de aceitação FST.
:::
...
O bloco provenance comunica o status de licenciamento dos recursos empacotados do plugin.
...
| `dictionary` | object | — | Mapa de termo → tradução. Os termos correspondentes são injetados como terminologia obrigatória. |
...
Os manifestos de plugin são validados no momento da instalação em relação a [`schemas/rosetta-plugin.schema.json`](https://github.com/gamedaysuits/i18n-rosetta/blob/main/schemas/rosetta-plugin.schema.json).
