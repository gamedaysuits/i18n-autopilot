...
Mỗi bản dịch đều đi qua một cổng xác thực tất định trước khi được ghi vào ổ đĩa. Quality gate này sẽ bắt các lỗi dịch máy phổ biến — không có dự phòng ngầm, không có dữ liệu rác nào được ghi vào các tệp locale của bạn.
...
Ngân sách thử lại được giới hạn bởi `maxRetries` (mặc định: 3, có thể cấu hình cho từng ngôn ngữ). Điều này ngăn chặn việc tiêu tốn token mất kiểm soát vào các key liên tục thất bại.
...
System message (ngữ điệu, quy tắc ngữ pháp, ghi chú văn phong) được tách khỏi user message (các key cần dịch). Việc tách biệt này là có chủ ý:

- System message **giống hệt nhau giữa các batch** cho một locale nhất định
- Các nhà cung cấp như Anthropic và Google sẽ lưu bộ nhớ đệm (cache) các system message lặp lại
...
Rosetta xác minh rằng các phiên bản đã dịch bao gồm tất cả các danh mục số nhiều bắt buộc cho locale đích. Ví dụ: tiếng Ả Rập yêu cầu sáu danh mục (`zero`, `one`, `two`, `few`, `many`, `other`) — chứ không chỉ `one` và `other`.

Chạy `i18n-rosetta integrity` để kiểm tra tính đầy đủ của số nhiều trên tất cả các locale.

## Thực thi thuật ngữ

Đối với các cặp ngôn ngữ được huấn luyện (coached pairs) có từ điển, rosetta sẽ chạy kiểm tra thuật ngữ sau khi dịch. Sau khi vượt qua quality gate, nó sẽ xác minh xem LLM có thực sự sử dụng các thuật ngữ từ điển được yêu cầu hay không.
...
Các vi phạm thuật ngữ là **cảnh báo, không phải lỗi chặn (blocking errors)**. Bản dịch vẫn được ghi vào ổ đĩa. Điều này là có chủ ý — LLM có thể có lý do hợp lệ để chọn một phương án thay thế (ngữ cảnh, ngữ pháp) và việc chặn do không khớp thuật ngữ sẽ gây hại nhiều hơn là có lợi.
...
- [Các phương pháp dịch](/docs/guides/translation-methods) — các phương pháp đưa dữ liệu vào cổng
...
- [Bộ nhớ dịch thuật](/docs/concepts/translation-memory) — lưu bộ nhớ đệm các bản dịch đã xác thực
