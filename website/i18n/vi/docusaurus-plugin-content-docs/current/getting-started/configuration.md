...
Khối cấu hình `typegen` được trình tải cấu hình nhận diện và giữ lại, nhưng tính năng tạo kiểu (type generation) TypeScript vẫn chưa được triển khai. Đây là phần giữ chỗ cho một tính năng đã được lên kế hoạch. Việc thiết lập các giá trị này hiện không có tác dụng.
:::
...
| `translatableFields` | `string[]` | `null` | Ghi đè các trường frontmatter có thể dịch mặc định cho việc dịch nội dung. `null` sử dụng các mặc định có sẵn (`title`, `description`, `summary`). |
...
| `batchSize` | `number` | `30` | Số lượng key trên mỗi lô dịch (batch). Cao hơn = ít lệnh gọi API hơn, nhưng prompt lớn hơn. |
| `concurrency` | `number` | `12` | Số lệnh gọi API song song tối đa cho việc dịch nội dung (Markdown/MDX). Bị ghi đè bởi cờ CLI `--concurrency`. |
| `fallbackPrefix` | `string` | `"[EN] "` | Tiền tố được thêm vào các giá trị dự phòng chưa được dịch. Được `audit` sử dụng để phát hiện các bản dịch chưa hoàn chỉnh. |
| `apiKeyEnvVar` | `string` | `"OPENROUTER_API_KEY"` | Tên biến môi trường cho API key. Ghi đè cho các tên biến môi trường tùy chỉnh. |
| `baseUrl` | `string` | `""` | Base URL để tạo các thành phần SEO (hreflang, sitemaps, JSON-LD). |
| `pairs` | `object` | `{}` | Ghi đè phương thức, mô hình và chất lượng cho từng cặp ngôn ngữ. Xem [Cấu hình cặp ngôn ngữ](#pair-configuration). |
...
| `typegen.autoGenerate` | `boolean` | `false` | Tự động tạo lại các kiểu (types) sau mỗi lần đồng bộ. |
...
Mỗi ngôn ngữ nhận văn phong (register) mặc định từ bảng văn phong có sẵn. Các ngôn ngữ không có mặc định sẽ nhận `"Professional register."`.

### Đối tượng với các chuỗi văn phong

Giá trị có thể là một **khóa cài đặt sẵn (preset key)** từ thẻ ngôn ngữ, hoặc văn bản văn phong tùy chỉnh:
...
Rosetta kiểm tra xem chuỗi có khớp với một khóa cài đặt sẵn trong thẻ ngôn ngữ hay không. Nếu có, toàn bộ prompt văn phong từ thẻ sẽ được sử dụng. Nếu không, chuỗi sẽ được sử dụng nguyên bản. Xem [Ngôn ngữ được hỗ trợ](/docs/reference/supported-languages#language-cards) để biết các cài đặt sẵn hiện có.
...
| Trường | Kiểu | Mô tả |
|-------|------|-------------|
| `register` | `string` | Hướng dẫn về phong cách/giọng điệu. Có thể là một **khóa cài đặt sẵn** (ví dụ: `casual-tu`, `formal-hapsyo`) hoặc văn bản tùy chỉnh. Xem [Thẻ ngôn ngữ](/docs/reference/supported-languages#language-cards). |
...
| `script` | `string` | Mã chữ viết (script code) ISO 15924. Kích hoạt xác thực chữ viết trong cổng chất lượng (quality gate). |

:::info Chuỗi kế thừa
Các cài đặt được phân giải theo thứ tự sau (ưu tiên cái đầu tiên):

**cấp độ cặp ngôn ngữ** → **cấp độ ngôn ngữ** → **cấu hình toàn cục** → **mặc định**
...
Rosetta tạo `.i18n-rosetta.lock` để theo dõi các mã băm SHA-256 của các giá trị nguồn đã dịch. **Hãy commit tệp này** để tất cả các nhà phát triển cùng chia sẻ chung một cơ sở bản dịch.

Khi một giá trị nguồn thay đổi, mã băm không còn khớp nữa và rosetta sẽ dịch lại key đó trong lần đồng bộ tiếp theo.
...
Rosetta tạo một thư mục `.rosetta/` trong thư mục gốc của dự án cho trạng thái nội bộ. Nhìn chung, bạn nên **thêm thư mục này vào `.gitignore`** — đây là tối ưu hóa cục bộ, không phải nguồn dự án:
...
| `xliff/*.xliff` | Các tệp xuất XLIFF để biên dịch viên chuyên nghiệp đánh giá | Không (tạm thời) |
| `methods/` | Các manifest của plugin phương thức đã cài đặt | Có (cấu hình dùng chung) |
| `backups/` | Các bản sao lưu trước khi wrap (được tạo bởi `wrap --undo`) | Không (mạng lưới an toàn) |

Xem [Translation Memory](/docs/concepts/translation-memory) để biết chi tiết về `tm.json` và cách nó tiết kiệm chi phí API.
...
Mở rộng `DirectLLMMethod` để thêm một nhà cung cấp LLM mới trong khoảng 40 dòng:
...
Bạn được cung cấp miễn phí các tính năng dịch, huấn luyện (coaching), vòng lặp thử lại (retry loops), xác thực mô hình, cấp độ chất lượng và trợ giúp thiết lập. Chỉ có cấu trúc của HTTP request là đặc thù cho từng nhà cung cấp. Đối với các adapter không phải LLM sử dụng `fetch()` thô, hãy sử dụng helper dùng chung `fetchWithRetry()` từ `lib/methods/fetch-with-retry.js` thay vì tự viết vòng lặp thử lại của riêng bạn.
...
- [Các phương thức dịch](/docs/guides/translation-methods) — chọn và kết hợp các phương thức
- [Translation Memory](/docs/concepts/translation-memory) — bộ nhớ đệm và tiết kiệm chi phí
- [Làm việc với biên dịch viên chuyên nghiệp](/docs/guides/professional-translators) — quy trình làm việc XLIFF
...
- [Ngôn ngữ được hỗ trợ](/docs/reference/supported-languages) — hỗ trợ ngôn ngữ có sẵn
