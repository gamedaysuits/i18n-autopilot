...
Rosetta hoạt động mà không cần cấu hình (zero-config) — nó tự động phát hiện các tệp locale, định dạng và ngôn ngữ đích từ dự án của bạn. Để kiểm soát nhiều hơn, hãy tạo `i18n-rosetta.config.json` trong thư mục gốc của dự án, hoặc chạy:
...
Khối cấu hình `typegen` được nhận diện và giữ lại bởi trình tải cấu hình, nhưng tính năng tạo type TypeScript vẫn chưa được triển khai. Đây là phần giữ chỗ cho một tính năng đã được lên kế hoạch. Việc thiết lập các giá trị này sẽ không có tác dụng gì.
:::
...
| `batchSize` | `number` | `80` | Số lượng key trên mỗi batch dịch. Cao hơn = ít lệnh gọi API hơn, nhưng prompt lớn hơn. |
| `jsonConcurrency` | `number` | `50` | Số lượng bản dịch locale song song tối đa cho việc đồng bộ key JSON. Bị ghi đè bởi cờ CLI `--json-concurrency`. |
| `contentConcurrency` | `number` | `12` | Số lượng lệnh gọi API song song tối đa cho việc dịch nội dung (Markdown/MDX). Bị ghi đè bởi cờ CLI `--content-concurrency`. |
...
| `baseUrl` | `string` | `""` | Base URL cho việc tạo các artifact SEO (hreflang, sitemaps, JSON-LD). |
| `pairs` | `object` | `{}` | Ghi đè phương thức, model và chất lượng cho từng cặp. Xem [Cấu hình cặp](#pair-configuration). |
...
| `typegen.autoGenerate` | `boolean` | `false` | Tự động tạo lại các type sau mỗi lần đồng bộ. |
...
Mỗi ngôn ngữ nhận register (văn phong) mặc định từ bảng register có sẵn. Các ngôn ngữ không có mặc định sẽ nhận `"Professional register."`.

### Object với các chuỗi register

Giá trị có thể là một **preset key** từ thẻ ngôn ngữ (language card), hoặc văn bản register tùy chỉnh:
...
Rosetta sẽ kiểm tra xem chuỗi có khớp với một preset key trong thẻ ngôn ngữ hay không. Nếu có, toàn bộ prompt register từ thẻ sẽ được sử dụng. Nếu không, chuỗi sẽ được sử dụng nguyên bản. Xem [Ngôn ngữ được hỗ trợ](/docs/reference/supported-languages#language-cards) để biết các preset có sẵn.
...
| `maxRetries` | `number` | Ngân sách thử lại tối đa cho các batch bị lỗi (mặc định: 3) |
| `script` | `string` | Mã script ISO 15924. Kích hoạt xác thực script trong quality gate. |
...
Rosetta tạo `.i18n-rosetta.lock` để theo dõi mã băm SHA-256 của các giá trị nguồn đã dịch. **Hãy commit tệp này** để tất cả các nhà phát triển chia sẻ cùng một cơ sở bản dịch.
...
| `xliff/*.xliff` | Các tệp xuất XLIFF để các dịch giả chuyên nghiệp đánh giá | Không (tạm thời) |
| `methods/` | Manifest của các plugin phương thức đã cài đặt | Có (cấu hình dùng chung) |
...
Đối với các script build và tích hợp tùy chỉnh, hãy import trực tiếp từ package:
...
| `validateTranslations` | Quality gate (Cổng kiểm soát chất lượng) |
...
Bạn nhận được các tính năng dịch, huấn luyện (coaching), vòng lặp thử lại (retry loop), xác thực model, cấp độ chất lượng và trợ giúp thiết lập miễn phí. Chỉ có cấu trúc HTTP request là đặc thù của từng nhà cung cấp. Đối với các adapter không phải LLM sử dụng `fetch()` thô, hãy sử dụng helper dùng chung `fetchWithRetry()` từ `lib/methods/fetch-with-retry.js` thay vì tự viết vòng lặp thử lại của riêng bạn.
...
- [Làm việc với các dịch giả chuyên nghiệp](/docs/guides/professional-translators) — quy trình làm việc XLIFF
