...
| `baseUrl` | `string` | `""` | URL cơ sở để tạo các tạo tác SEO (hreflang, sitemaps, JSON-LD). |
...
| `seo.urlPattern` | `string` | `"/:locale/:path"` | Mẫu URL template để tạo thẻ hreflang. |
...
| `typegen.autoGenerate` | `boolean` | `false` | Tự động tạo lại các kiểu sau mỗi lần đồng bộ (sync). |
...
### Mảng mã ngôn ngữ (đơn giản nhất)
...
Mỗi ngôn ngữ sẽ nhận văn phong (register) mặc định từ bảng văn phong có sẵn. Các ngôn ngữ không có mặc định sẽ nhận `"Professional register."`.

### Đối tượng với chuỗi văn phong
...
Rosetta sẽ kiểm tra xem chuỗi có khớp với một khóa cài đặt sẵn trong thẻ ngôn ngữ hay không. Nếu có, toàn bộ prompt văn phong từ thẻ sẽ được sử dụng. Nếu không, chuỗi sẽ được sử dụng nguyên bản. Xem [Ngôn ngữ được hỗ trợ](/docs/reference/supported-languages#language-cards) để biết các cài đặt sẵn có sẵn.
...
Bạn có thể kết hợp cả dạng viết tắt và đối tượng đầy đủ trong cùng một khối.
...
| `maxRetries` | `number` | Số lần thử lại tối đa cho các lô bị lỗi (mặc định: 3) |
| `script` | `string` | Mã hệ thống chữ viết ISO 15924. Kích hoạt xác thực hệ thống chữ viết trong cổng chất lượng (quality gate). |
...
Rosetta tạo `.i18n-rosetta.lock` để theo dõi mã băm SHA-256 của các giá trị nguồn đã dịch. **Hãy commit tệp này** để tất cả các nhà phát triển cùng chia sẻ một cơ sở bản dịch chung.
...
Xem [Bộ nhớ dịch (Translation Memory)](/docs/concepts/translation-memory) để biết chi tiết về `tm.json` và cách nó tiết kiệm chi phí API.
...
Mở rộng `DirectLLMMethod` để thêm một nhà cung cấp LLM mới trong khoảng 40 dòng mã:
...
Bạn được cung cấp sẵn các tính năng dịch, huấn luyện, vòng lặp thử lại, xác thực mô hình, cấp độ chất lượng và trợ giúp thiết lập hoàn toàn miễn phí. Chỉ có cấu trúc của HTTP request là đặc thù cho từng nhà cung cấp. Đối với các adapter không phải LLM sử dụng `fetch()` thô, hãy sử dụng helper dùng chung `fetchWithRetry()` từ `lib/methods/fetch-with-retry.js` thay vì tự viết vòng lặp thử lại của riêng bạn.
...
- [Các phương thức dịch](/docs/guides/translation-methods) — cách chọn và kết hợp các phương thức
- [Bộ nhớ dịch (Translation Memory)](/docs/concepts/translation-memory) — bộ nhớ đệm và tiết kiệm chi phí
