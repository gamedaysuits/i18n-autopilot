...
Rosettaはゼロコンフィグで動作します。プロジェクトからロケールファイル、フォーマット、ターゲット言語を自動検出します。より詳細な制御を行うには、プロジェクトのルートに `i18n-rosetta.config.json` を作成するか、次のコマンドを実行します。
...
| `translatableFields` | `string[]` | `null` | コンテンツ翻訳におけるデフォルトの翻訳可能なフロントマターのフィールドを上書きします。`null` は組み込みのデフォルト (`title`, `description`, `summary`) を使用します。 |
...
| `batchSize` | `number` | `30` | 翻訳バッチごとのキー数。数値を大きくするとAPI呼び出し回数は減りますが、プロンプトが大きくなります。 |
| `concurrency` | `number` | `12` | コンテンツ (Markdown/MDX) 翻訳における最大並列API呼び出し数。`--concurrency` CLIフラグによって上書きされます。 |
...
| `baseUrl` | `string` | `""` | SEOアーティファクト生成 (hreflang、サイトマップ、JSON-LD) のためのベースURL。 |
...
| `lint.srcDir` | `string` | `null` | lintスキャンのソースディレクトリ。`null` = フレームワークから自動検出。 |
| `lint.ignore` | `string[]` | `["node_modules", ...]` | lintから除外するglobパターン。 |
| `lint.minLength` | `number` | `2` | ハードコードとしてフラグを立てる最小文字列長。 |
| `seo.urlPattern` | `string` | `"/:locale/:path"` | hreflangタグ生成のためのURLパターンテンプレート。 |
| `seo.pages` | `string[]` | `null` | SEOのための明示的なページリスト。`null` = ロケールキーから自動検出。 |
| `typegen.output` | `string` | `null` | 生成されたTypeScript型の出力パス。`null` = 無効。 |
| `typegen.autoGenerate` | `boolean` | `false` | 同期ごとに型を自動再生成します。 |
...
| `endpoint` | `string` | リモートAPIエンドポイントのURL。`method` が `api` の場合に必須です。 |
| `qualityTier` | `string` | 表示ティア: `standard`, `high`, `research`, `verified` |

## 言語設定

言語設定では3つのフォーマットを受け付けます。
...
各言語は組み込みのレジスターテーブルからデフォルトのレジスター（文体・トーン）を取得します。デフォルトがない言語には `"Professional register."` が適用されます。
...
Rosettaは、文字列が言語カードのプリセットキーと一致するかどうかを確認します。一致する場合は、カードの完全なレジスタープロンプトが使用されます。一致しない場合は、文字列がそのまま使用されます。利用可能なプリセットについては、[サポートされている言語](/docs/reference/supported-languages#language-cards) を参照してください。
...
| `script` | `string` | ISO 15924のスクリプトコード。品質ゲートでのスクリプト検証をトリガーします。 |

:::info 継承チェーン
設定は以下の順序で解決されます (最初に一致したものが優先されます):

**ペアレベル** → **言語レベル** → **グローバル設定** → **デフォルト**

例えば、`pairs["en:fr"]` で `model` を設定した場合、言語レベルおよびグローバルの `model` の値の両方を上書きします。
:::
...
プロジェクトのルートに `.rosettaignore` を作成して、ファイルを `lint` のスキャンから除外します。`.gitignore` のようなglobパターンを使用します。
...
Rosettaは、内部状態を保持するためにプロジェクトのルートに `.rosetta/` ディレクトリを作成します。これはプロジェクトのソースではなくローカルの最適化であるため、通常は**これを `.gitignore` に追加**する必要があります。
...
| ファイル | 目的 | コミット? |
|------|---------|--------|
| `tm.json` | 翻訳メモリのキャッシュ — ソーステキスト + ロケール + メソッドをキーとして以前の翻訳を保存します | いいえ (ローカルキャッシュ) |
...
| `LLMMethod` | LLMメソッドの基底クラス (OpenRouter) |
| `DirectLLMMethod` | 直接のLLMプロバイダーの基底クラス (OpenAI, Anthropic, Gemini) |
...
`DirectLLMMethod` を拡張することで、約40行で新しいLLMプロバイダーを追加できます。
...
翻訳、コーチング、再試行ループ、モデルの検証、品質ティア、およびセットアップのヘルプが標準で提供されます。プロバイダー固有のものはHTTPリクエストの形式のみです。生の `fetch()` を使用する非LLMアダプターの場合は、独自の再試行ループを記述する代わりに、`lib/methods/fetch-with-retry.js` の共有ヘルパー `fetchWithRetry()` を使用してください。
...
- [アーキテクチャ](/docs/concepts/architecture) — 各コンポーネントの連携方法
