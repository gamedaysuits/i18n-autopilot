...
Rosettaはゼロコンフィグで動作します。プロジェクトからロケールファイル、フォーマット、ターゲット言語を自動検出します。より詳細な制御を行うには、プロジェクトのルートに `i18n-rosetta.config.json` を作成するか、以下のコマンドを実行します。
...
| `translatableFields` | `string[]` | `null` | コンテンツ翻訳における、翻訳可能なデフォルトのフロントマターフィールドを上書きします。`null` の場合は組み込みのデフォルト (`title`, `description`, `summary`) を使用します。 |
...
| `baseUrl` | `string` | `""` | SEOアーティファクト (hreflang、サイトマップ、JSON-LD) 生成用のベースURL。 |
| `pairs` | `object` | `{}` | ペアごとのメソッド、モデル、品質の上書き。詳細は [ペアの設定](#pair-configuration) を参照してください。 |
| `languages` | `object` | `{}` | 言語ごとの上書き。詳細は [言語の設定](#language-configuration) を参照してください。 |
...
| `typegen.autoGenerate` | `boolean` | `false` | 同期のたびに型を自動再生成します。 |
...
各言語は、組み込みのレジスターテーブルからデフォルトのレジスターを取得します。デフォルトがない言語には `"Professional register."` が適用されます。
...
たとえば、`pairs["en:fr"]` で `model` を設定した場合、言語レベルおよびグローバルの `model` の値の両方を上書きします。
:::
...
`lint` のスキャンからファイルを除外するには、プロジェクトのルートに `.rosettaignore` を作成します。`.gitignore` のようにGlobパターンを使用します:
...
翻訳、コーチング、再試行ループ、モデル検証、品質ティア、セットアップのヘルプをそのまま利用できます。プロバイダー固有なのはHTTPリクエストの形式のみです。生の `fetch()` を使用する非LLMアダプターの場合は、独自の再試行ループを記述する代わりに、`lib/methods/fetch-with-retry.js` の共有 `fetchWithRetry()` ヘルパーを使用してください。
