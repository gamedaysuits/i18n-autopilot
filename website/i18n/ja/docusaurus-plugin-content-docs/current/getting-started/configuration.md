...
Rosettaは、文字列が言語カードのプリセットキーと一致するかどうかを確認します。一致する場合、カードの完全なレジスタープロンプトが使用されます。一致しない場合は、文字列がそのまま使用されます。利用可能なプリセットについては、[Supported Languages](/docs/reference/supported-languages#language-cards) を参照してください。
...
| フィールド | 型 | 説明 |
|-------|------|-------------|
| `register` | `string` | スタイル/トーンの指示。**プリセットキー** (例: `casual-tu`、`formal-hapsyo`) またはカスタムテキストを指定できます。[Language Cards](/docs/reference/supported-languages#language-cards) を参照してください。 |
...
`tm.json` の詳細と、それがどのようにAPIコストを節約するかについては、[Translation Memory](/docs/concepts/translation-memory) を参照してください。
...
- [CLI Reference](/docs/reference/cli) — すべてのコマンドとフラグ
- [Translation Methods](/docs/guides/translation-methods) — メソッドの選択と組み合わせ
...
- [Plugin Specification](/docs/reference/plugin-spec) — メソッドプラグインのマニフェストフォーマット
- [Architecture](/docs/concepts/architecture) — 各コンポーネントの連携方法
- [Supported Languages](/docs/reference/supported-languages) — 組み込みの言語サポート
- [How Sync Works](/docs/concepts/how-sync-works) — 翻訳パイプライン