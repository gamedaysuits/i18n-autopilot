...
| `translatableFields` | `string[]` | `null` | コンテンツ翻訳用に、デフォルトの翻訳可能なフロントマターフィールドを上書きします。`null` は組み込みのデフォルト (`title`、`description`、`summary`) を使用します。 |
...
| `jsonConcurrency` | `number` | `50` | JSONキー同期における、ロケール翻訳の最大並列数。`--json-concurrency` CLIフラグによって上書きされます。 |
| `contentConcurrency` | `number` | `12` | コンテンツ (Markdown/MDX) 翻訳における、API呼び出しの最大並列数。`--content-concurrency` CLIフラグによって上書きされます。 |
| `fallbackPrefix` | `string` | `"[EN] "` | 以前の実行から残っている未翻訳のレガシー値を検出するために、`audit` と `verify` が使用するマーカープレフィックス。Rosettaはこのプレフィックスを書き込まず、検出のために読み取るのみです。 |
...
各言語は、組み込みのレジスターテーブルからデフォルトのレジスター（トーン）を取得します。デフォルトがない言語は `"Professional register."` になります。
...
Rosettaは、文字列が言語カードのプリセットキーと一致するかどうかを確認します。一致する場合、カードの完全なレジスタープロンプトが使用されます。一致しない場合、文字列はそのまま使用されます。利用可能なプリセットについては、[サポートされている言語](/docs/reference/supported-languages#language-cards) を参照してください。
...
たとえば、`pairs["en:fr"]` が `model` を設定した場合、言語レベルおよびグローバルの `model` 値の両方を上書きします。
:::
...
Rosettaは、内部状態を保持するためにプロジェクトのルートに `.rosetta/` ディレクトリを作成します。これはプロジェクトのソースではなくローカルの最適化であるため、通常は**これを `.gitignore` に追加する**必要があります。
...
`DirectLLMMethod` を拡張して、約40行で新しいLLMプロバイダーを追加します。
...
- [アーキテクチャ](/docs/concepts/architecture) — 各コンポーネントの接続方法
