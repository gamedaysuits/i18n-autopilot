...
description: "複雑な翻訳パイプライン（FSTゲート、マルチステップLLMチェーン）をHTTPサービスとしてラップし、apiメソッドを通じてi18n-rosettaに組み込みます。"
---
# カスタムメソッドをAPIとして提供する

i18n-rosettaの**`api` method**を使用すると、任意の翻訳ペアを外部のHTTPエンドポイントに向けることができます。これにより、形態素解析器、有限状態トランスデューサー（FST）、複数ステップのLLMチェーン、または独自に構築したカスタム研究メソッドなど、単一のLLMプロンプトでは複雑すぎるパイプラインを統合できます。
...
| パイプラインのステップ | 例 |
|---|---|
| **形態素分解** | 翻訳前に複統合的な単語を形態素に分割する |
| **FST検証** | 音韻規則や形態素規則に違反する出力を拒否する |
| **複数ステップのLLMチェーン** | 異なるモデルを使用して生成 → 検証 → 修正のサイクルを行う |
| **辞書検索** | パイプラインの途中で精選された対訳辞書を相互参照する |
...
rosettaは、以下のJSONボディをそのまま送信します（[api.js](https://github.com/gamedaysuits/i18n-rosetta/blob/main/lib/methods/api.js)を参照してください）。
...
1. **失敗した場合は空の文字列を返す** — ソース文字列を「翻訳」として返さないでください。`""`を返し、i18n-rosettaのフォールバックプレフィックスメカニズムに処理を任せます。
2. **信頼度スコアを含める** — パイプラインが品質を推定できる場合は、メタデータで返します。これは品質監査に役立ちます。
3. **ヘルスチェックを実装する** — 大規模な同期を開始する前にi18n-rosettaが接続を確認できるように、`GET /health`エンドポイントを追加します。
4. **適切にレート制限を行う** — パイプラインにスループット制限がある場合は、`429`ステータスコードを返します。i18n-rosettaのバッチシステムはバックオフ（待機）します。
5. **すべてをログに記録する** — 複数ステップのパイプラインは、エラーを出さずに失敗する（サイレントフェイル）可能性があります。デバッグのために各ステップの入出力をログに記録してください。
...
- [低リソース言語のサポート](/docs/guides/low-resource-languages) — OCAP原則を含む、リソースが不足している言語向けのエンドツーエンドガイド
- [アーキテクチャ](/docs/concepts/architecture) — i18n-rosettaの同期ループ、バッチ処理、メソッドディスパッチの仕組み
- [機械翻訳（MT）の評価](/docs/eval/) — 評価手法、指標、およびリーダーボードへの提出プロセス
