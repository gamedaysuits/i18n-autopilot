...
Rosetta翻訳エコシステムは、明確に定義されたコントラクトを通じて連携する3つの独立したツールで構成されています。ビルド時に互いに依存するものはありません。これらは、共有の**メソッドプラグインフォーマット**と**REST APIコントラクト**を通じて通信します。
...
**組み込みメソッド:**
- `llm` → OpenRouter / 任意のLLM
- `llm-coached` → LLM + 文法/辞書コーチング
- `google-translate` → Google Cloud Translation API
- `api` → 任意のリモートAPIへのシンパイプ
...
独自の翻訳メソッドをサーバーサイドでホストする従量課金制のAPIサービスです。プロンプト、コーチングデータ、言語パイプラインがサーバーから外部に出ることはありません。
...
Rosettaの`APIMethod`は**ダムパイプ**です。キーを送信し、翻訳を受け取ります。翻訳ロジックや独自のコンテンツは一切含まれていません。

## 各構成要素の相互認識について

| ツール | rosettaを知っているか？ | Rosetta Translateを知っているか？ | harnessを知っているか？ |
|------|---------------------|-------------------------------|---------------------|
...
| **Eval Harness** | はい — プラグインフォーマットをエクスポートします | いいえ — メソッドは個別にデプロイされます | *(harness自身)* |
...
組み込みの`google-translate`メソッドを使用します。プラグインは必要ありません。
...
ユーザーは独自の文法ルールと辞書を`.rosetta/coaching/fr.json`で管理します。

## 設計原則

1. **循環依存なし。** ブリッジは一方向です。
2. **Rosettaは軽量なコア。** 依存関係ゼロ、設定は任意です。プラグインとAPIは追加要素です。
3. **IP保護はアーキテクチャレベル。** 独自の技術はサーバーサイドに留まります。npmパッケージには独自のものは一切含まれません。
4. **プラグインフォーマットがコントラクト。** すべては`method.json`を通じて流れます。
