...
Ang eval harness at i18n-rosetta ay nagse-share ng common concept na **translation method**. Ang isang method ay anumang procedure na kumukuha ng source text at nagpo-produce ng translated text — maging ito man ay isang direct LLM call, isang multi-stage pipeline, isang third-party API, o isang human translator.
...
| **Output** | Run card na may metrics | Translated na locale files |

Ang isang method na nagsu-support sa parehong system ay nagpo-provide ng dalawang entry point — isa para sa bawat language runtime. Ang **method card** ang nagsisilbing tulay: dini-describe nito ang method sa isang format na naiintindihan ng parehong system.

## Method Card

Dini-describe ng isang method card kung *ano* ang isang translation method nang hindi nire-reveal ang mga proprietary detail tulad ng buong system prompt. Sinasagot nito ang mga sumusunod:
...
| `human` | Human translation (para sa pag-establish ng mga baseline) |

## Eval Harness: TranslationProcess Protocol

Gumagamit ang eval harness ng structural typing ng Python (`Protocol`) para sa mga plugin. Gagana ang anumang class na may tamang method signature — hindi required ang inheritance:
...
Tingnan ang [Plugin Protocol](https://github.com/gamedaysuits/gds-mt-eval-harness/blob/main/docs/plugin-protocol.md) para sa kumpletong documentation kasama ang mga wrapper example para sa mga non-Python method.

## i18n-rosetta: methodPlugin Config

Sa rosetta, naka-register ang mga method per language pair sa `i18n-rosetta.config.json`:
...
Kapag walang naka-attach na method card, ipinapakita ng leaderboard ang harness-native configuration (model, condition, temperature, tools enabled).

:::danger DO NOT TRAIN sa evaluation data
Ang mga method na ang development process ay na-expose sa evaluation dataset — bilang training data, few-shot examples, dictionary entries, o prompt tuning material — ay madi-**disqualify** sa leaderboard. Tingnan ang [MT Evaluation](/docs/eval/) para malaman kung ano ang pinagkaiba ng magandang method sa masama.
:::
...
- [Eval Harness](/docs/eval/harness) — kung paano mag-run ng mga evaluation
- [Evaluation Datasets](/docs/eval/datasets) — mga available na dataset (EDTeKLA, FLORES+)
