...
Ang mga script converter ay mga deterministic at LLM-free na post-translation hooks na nagko-convert ng text mula sa isang writing system patungo sa iba. Nag-e-enable sila ng "translate once, render in multiple scripts" na workflow — magta-translate ka sa isang working script (kadalasan ay Latin), tapos automatic itong magko-convert sa display script.

## Bakit Script Converters?

Ilang mga language ang gumagamit ng multiple scripts para sa iisang spoken language:
...
Ang pag-translate nang direkta sa mga non-Latin script ay nagdudulot ng mga problema: nagha-hallucinate ng mga character ang mga LLM, nagiging mahirap i-version-control ang mga JSON file, at hindi ma-compare ng mga diff tool ang mga changes. Sinu-solve ito ng mga script converter sa pamamagitan ng pag-keep ng mga translation sa isang version-control-friendly na script at pag-convert nang deterministic tuwing sync time.
...
Pare-pareho ang algorithm na ginagamit ng lahat ng converter: sa bawat character position, ita-try muna ang longest possible match, tapos progressively shorter matches. Ang mga character na hindi nagma-match sa kahit anong pattern (spaces, punctuation, numbers) ay magpa-pass through nang walang pagbabago.
...
## Paggamit ng Script Converters

Automatic na nag-a-activate ang mga script converter kapag nag-match ang locale code sa isang registered converter. Hindi na kailangan ng configuration — i-set lang ang inyong target locale:
...
Kapag nag-sync ang rosetta sa `en:crk` pair, ang mga translation ay pino-produce muna sa SRO, tapos automatic na iko-convert sa Syllabics bago i-write sa `crk.json`.
...
- [Cookbook: FST-Gated Pipeline](/docs/tutorials/fst-gated-pipeline) — script conversion sa isang multi-stage pipeline