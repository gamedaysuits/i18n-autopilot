...
Sinusuportahan ng Rosetta ang sampung translation methods. Pwedeng gumamit ng magkaibang method ang bawat language pair — hindi ka po naka-lock sa isang approach lang para sa buong project niyo.

## Pagkukumpara ng mga Method

### Mga LLM Provider

Quality-focused, Markdown-aware, at coaching-compatible. Pinaka-best para sa mga content-heavy na projects.
...
| `gemini` | `GEMINI_API_KEY` | Direct Google Gemini API (Flash, Pro) — may free tier |

### Traditional MT

Naka-focus sa speed at cost. Pinaka-best para sa high-volume na key-value pairs.
...
Nagta-translate gamit ang kahit anong LLM sa [OpenRouter](https://openrouter.ai). Ito po ang default method at ang pinaka-versatile.

**Paano ito gumagana:**
1. Iba-batch ang mga keys (default 30/batch) kasama ang register at context instructions
2. Ise-send sa OpenRouter bilang isang structured prompt
3. Ipa-parse ang JSON response
4. Iba-validate ang bawat translation gamit ang [quality gate](/docs/concepts/quality-gate)
5. Isa-save ang mga pumasa na translations, magre-retry o ire-reject ang mga nag-fail
...
**Paano ito gumagana:**
1. Ilo-load ang coaching data mula sa `.rosetta/coaching/<locale>.json` o sa `coaching/` directory ng isang plugin
2. Ii-inject ang grammar rules, dictionary terms, at style notes sa system prompt
3. Ang mga dictionary terms na nagma-match sa source keys ay isasama bilang required terminology
4. Magpapatuloy ang translation tulad ng sa `llm`, kung saan ang coaching data ay nagdadagdag ng precision
...
Direktang nagta-translate via OpenAI Chat Completions API. Walang OpenRouter middleman — sarili mong key, account mo, at usage dashboard mo.
...
Direktang nagta-translate via Anthropic Messages API. Ginagamit ang `system` parameter para sa coaching data, na nag-e-enable sa prompt caching ng Anthropic.
...
Direktang nagta-translate via Google Gemini `generateContent` API. **May free tier na available** — ang pinaka-best na zero-cost starting point.
...
- ✅ Free tier (generous na daily quota)
- ✅ Exponential backoff na may retry
...
Bina-validate ng mga direct LLM providers (`openai`, `anthropic`, `gemini`) ang inyong model string sa unang gamit. Naka-catch nito ang tatlong categories ng pagkakamali:
...
Nalo-log ang model validation ng mga warnings pero hindi nito bina-block ang API call. Ang provider API pa rin ang magbibigay ng final verdict — pwedeng mag-match ang isang future model name sa ibang pattern, at ayaw po nating mag-gate base sa heuristics.
:::
...
Direct integration sa Google Cloud Translation API v2. Gumagamit ng REST API — walang SDK, walang service account. API key lang po.
...
**Mga Limitasyon:**
- ⚠️ **Walang Markdown awareness.** Mako-corrupt nito ang mga code blocks, shortcodes, at interpolation variables.
...
:::tip Auto-detection
Kung `GOOGLE_TRANSLATE_API_KEY` lang ang naka-set (walang OpenRouter key), mag-a-auto-switch ang rosetta sa Google Translate. Walang kailangang baguhin sa config.
:::

## `deepl` — DeepL API

Direct integration sa DeepL translation API. May support para sa mga glossaries para sa consistent na terminology.

**Kailan ito gagamitin:** Sa mga European languages kung saan magaling ang DeepL (German, French, Spanish, Dutch, Polish, atbp.). Ang glossary support ay nag-e-enforce ng consistent na terminology kahit walang coaching data.
...
**Kailan ito gagamitin:** Sa mga enterprise environments na may existing na Azure infrastructure. Sinusuportahan ang 100+ languages kasama ang marami na hindi sakop ng Google Translate.
...
Self-hosted na open-source translation gamit ang LibreTranslate. Nagra-run locally o sa sarili mong infrastructure — zero API costs, full data sovereignty.

**Kailan ito gagamitin:** Sa mga projects na kailangan ng offline translation, data privacy compliance (GDPR), o zero-cost operation. Lalo na itong useful para sa mga CI pipelines na hindi dapat dumedepende sa external APIs.
...
Isang thin HTTP client para sa mga community-hosted o IP-protected na translation endpoints. Nagse-send ang Rosetta ng mga keys at tumatanggap ng mga translations pabalik — wala itong kahit anong translation logic.
...
Ang totoong power nito ay ang pag-mix ng mga methods per language pair:
...
Ita-translate nito ang French via DeepL (glossary support), Japanese via OpenAI (quality), Korean via Gemini (free tier), Arabic via Microsoft Translator (coverage), at Plains Cree via isang coached plugin (specialized).
...
Lilipat sa ibang method? Magbabago ang model format at env var — heto po ang map:
...
- Gumagamit ang OpenRouter ng `provider/model` format (hal., `openai/gpt-4o`). Ang mga direct providers ay gumagamit ng bare model names (hal., `gpt-4o`).
- May sariling env var ang bawat direct provider (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`).
- Kung maling model format ang ginamit niyo, magwa-warn ang rosetta sa inyo — tingnan ang [Model Validation](#model-validation).
...
**Gamitin ang OpenRouter** kung gusto niyong mag-switch ng models nang hindi nagpapalit ng env vars, o kung gusto niyo ng access sa 200+ models gamit ang isang key lang. **Gamitin ang direct providers** kung gusto niyo ng mas simpleng billing, lower latency (walang middleman), o access sa mga provider-specific features tulad ng prompt caching ng Anthropic.
:::
...
| Method | Cost / 1K Keys | Speed | Quality | Pinaka-best Para Sa |
|--------|----------------|-------|---------|----------|
| `gemini` (Flash) | **Libre** (sa loob ng tier) | Mabilis | Maganda | Getting started, personal projects |
| `google-translate` | ~$0.02 | Pinakamabilis | Sapat | High-volume, European languages |
| `deepl` | ~$0.02 | Mabilis | Maganda | European languages, terminology |
...
| `gemini` (Pro) | ~$0.07 | Katamtaman | Napakaganda | Quality-sensitive, free quota |
| `openai` (GPT-4o-mini) | ~$0.01 | Mabilis | Maganda | Budget LLM |
| `openai` (GPT-4o) | ~$0.10 | Katamtaman | Napakaganda | Quality-sensitive |
| `anthropic` (Haiku) | ~$0.01 | Mabilis | Maganda | Budget LLM |
| `anthropic` (Sonnet) | ~$0.10 | Katamtaman | Napakaganda | Quality-sensitive |
| `anthropic` (Opus) | ~$0.50 | Mabagal | Excellent | Maximum quality |
| `llm` (OpenRouter) | Nag-iiba per model | Nag-iiba | Nag-iiba | Model comparison, experimentation |

:::note Mga estimates po ito
Nakadepende ang actual costs sa haba ng source text niyo, batch size, at mga pagbabago sa pricing ng provider. I-check ang current pricing page ng bawat provider para sa mga eksaktong rates.
:::
