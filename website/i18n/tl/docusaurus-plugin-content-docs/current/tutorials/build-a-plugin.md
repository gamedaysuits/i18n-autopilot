...
description: "End-to-end tutorial: i-develop ang coaching data, i-benchmark gamit ang eval harness, i-export ang plugin, at i-deploy ito gamit ang rosetta."
---
# Tutorial: Gumawa ng Translation Plugin

Bumuo ng custom translation method from scratch, i-benchmark ito, at i-deploy bilang isang rosetta plugin. Ito po ang kumpletong workflow para mag-add ng bagong language pair na hindi sinusuportahan ng kahit anong off-the-shelf API.
...
Nagta-translate po kayo ng isang SaaS dashboard sa French. Ang default na `llm` method ay nagpo-produce ng tama pero inconsistent na translations:
...
- Ang mga technical terms ay nagiging anglicized nang inconsistent

Kailangan ninyo ng **terminology enforcement** at **register control** na hindi naipo-provide ng isang generic na LLM prompt.
...
- **`style_notes`** — Naka-append sa system prompt bilang general style guidance

## Step 3: I-configure ang Pair

Sabihan ang rosetta na gamitin ang `llm-coached` para sa French:
...
I-review po ang dry-run output. I-check kung:
...
## Step 5: Mag-benchmark gamit ang Eval Harness (Optional)

Kung gusto ninyo ng quality scores — at siguradong gusto ninyo, dahil ang mga plugins ay nagshi-ship kasama ng benchmark data — gamitin po ang companion eval harness.
...
- **Exact match rate** — Proportion ng mga translations na eksaktong nagma-match sa reference.
...
2. **Benchmark scores** — Quantified quality na nagshi-ship kasama ng plugin
