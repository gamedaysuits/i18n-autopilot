...
I-translate po ang inyong unang locale file in 60 seconds.
...
Kumuha po ng free Gemini key sa [aistudio.google.com/apikey](https://aistudio.google.com/apikey). Kumuha ng OpenRouter key sa [openrouter.ai](https://openrouter.ai).
...
1. I-auto-detect ang `locales/en.json` as the source
2. Hanapin (o mag-prompt para sa) target languages
3. I-translate ang lahat ng keys
4. I-write ang `locales/fr.json`, `locales/ja.json`, etc.
...
Kapag pinalitan niyo po ang isang source string, ide-detect ng rosetta ang change via SHA-256 hash tracking at ire-re-translate lang ang key na iyon sa next sync:
...
For more control, mag-generate po ng config file:
...
Iga-guide po kayo ng wizard sa **register presets** ng bawat language — mga pre-built tone/formality instructions na naka-tune sa linguistic system nito. Ang French ay may T-V presets (vouvoiement vs tutoiement), ang Korean ay may speech levels (해요체 vs 합쇼체 vs 해체), at ang Japanese ay may keigo options (です/ます vs 丁寧語).

O kaya naman ay gumawa ng config manually gamit ang preset keys:
...
I-run po ang `npx i18n-rosetta init` para i-browse ang available presets for each language.

## Optional: Watch Mode

Mag-auto-translate kapag may nag-change sa inyong source file:
