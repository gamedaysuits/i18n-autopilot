...
Gumagana ang Rosetta nang zero-config — nag-a-auto-detect ito ng locale files, format, at target languages mula sa project ninyo. Para sa mas maraming control, gumawa po ng `i18n-rosetta.config.json` sa project root ninyo, o i-run ang:
...
Ang `typegen` config block ay nire-recognize at pini-preserve ng config loader, pero hindi pa po implemented ang TypeScript type generation. Placeholder po ito para sa isang planned feature. Wala pong epekto ang pag-set ng mga values na ito.
:::
...
| `batchSize` | `number` | `30` | Keys per translation batch. Mas mataas = mas kaunting API calls, pero mas malaking prompts. |
...
| `typegen.output` | `string` | `null` | Output path para sa generated TypeScript types. `null` = naka-disable. |
| `typegen.autoGenerate` | `boolean` | `false` | I-auto-regenerate ang types pagkatapos ng bawat sync. |
...
Tumatanggap ng tatlong formats ang languages:
...
Tinitingnan ng Rosetta kung nagma-match ang string sa isang preset key sa language card. Kung oo, gagamitin ang buong register prompt mula sa card. Kung hindi, gagamitin ang string as-is. Tingnan ang [Supported Languages](/docs/reference/supported-languages#language-cards) para sa mga available na presets.
...
Pwede ninyong paghaluin ang shorthand at full objects sa iisang block.
...
| `script` | `string` | ISO 15924 script code. Nagti-trigger ito ng script validation sa quality gate. |
...
- [Architecture](/docs/concepts/architecture) — paano nagko-connect ang mga bahagi
