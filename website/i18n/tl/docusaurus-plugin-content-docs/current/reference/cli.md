...
**`--langs` option**: Comma-separated list ng mga target language code. Ini-skip nito ang language prompt at ina-apply ang default register presets para sa bawat language. I-combine sa `--yes` para sa fully non-interactive na setup.

**Language presets**: Kapag na-prompt para sa target languages, pwede mong i-type ang mga preset name:
...
**Change detection**: Nag-i-store ang rosetta ng SHA-256 hashes sa `.i18n-rosetta.lock`. Kapag nagbago ang source values, automatic na ire-re-translate ng susunod na sync ang mga keys na iyon. I-commit ang lock file para ma-share ng lahat ng developers ang baseline.

---

## watch

Nag-o-auto-sync kapag may nagbago sa source locale file. Magra-run ito hanggang ma-interrupt gamit ang `Ctrl+C`.
...
Inili-list ang lahat ng untranslated na `[EN]`-prefixed fallback values. Mag-e-exit ito with code 1 kapag may na-detect — gamitin ito bilang CI gate para i-fail ang mga build na may incomplete translations.
...
Ini-scan ang source code para sa mga hardcoded user-facing strings na dapat gumagamit ng i18n translation calls. Ina-auto-detect nito ang iyong framework (next-intl, react-i18next, vue-i18n, Hugo).
...
- Dead keys — mga locale key na hindi naka-reference sa kahit anong source file
...
Mag-generate ng SEO artifacts para sa multilingual sites.
...
**Ano ang chine-check nito:**
- Placeholder corruption (hal., `{name}` na nasa source pero missing sa target)
