...
Rosetta funktioniert ohne Konfiguration (Zero-Config) — es erkennt Lokalisierungsdateien, Format und Zielsprachen automatisch aus Ihrem Projekt. Für mehr Kontrolle erstellen Sie `i18n-rosetta.config.json` im Stammverzeichnis Ihres Projekts oder führen Sie Folgendes aus:
...
Der Konfigurationsblock `typegen` wird vom Konfigurationslader erkannt und beibehalten, aber die TypeScript-Typengenerierung ist noch nicht implementiert. Dies ist ein Platzhalter für eine geplante Funktion. Das Festlegen dieser Werte hat keine Auswirkungen.
:::
...
| `inputLocale` | `string` | `"en"` | Quellsprachencode (BCP 47). |
| `localesDir` | `string` | `"./locales"` | Pfad zu den Lokalisierungsdateien. Rosetta durchsucht dieses Verzeichnis. |
| `contentDir` | `string` | `null` | Hugo-Inhaltsverzeichnis. Aktiviert die Übersetzung des Markdown-Textkörpers. |
...
| `lint.srcDir` | `string` | `null` | Quellverzeichnis für Lint-Scans. `null` = automatische Erkennung aus dem Framework. |
...
| `seo.pages` | `string[]` | `null` | Explizite Seitenliste für SEO. `null` = automatische Erkennung aus Lokalisierungsschlüsseln. |
| `typegen.output` | `string` | `null` | Ausgabepfad für generierte TypeScript-Typen. `null` = deaktiviert. |
| `typegen.autoGenerate` | `boolean` | `false` | Automatische Neugenerierung von Typen nach jeder Synchronisierung. |
...
| Feld | Typ | Beschreibung |
|-------|------|-------------|
| `register` | `string` | Stil-/Tonanweisungen. Kann ein **Voreinstellungsschlüssel** (z. B. `casual-tu`, `formal-hapsyo`) oder benutzerdefinierter Text sein. Siehe [Sprachkarten](/docs/reference/supported-languages#language-cards). |
...
Wenn beispielsweise `pairs["en:fr"]` den Wert `model` festlegt, überschreibt dies sowohl die `model`-Werte auf Sprachebene als auch die globalen Werte.
:::
...
Rosetta erstellt `.i18n-rosetta.lock`, um SHA-256-Hashes der übersetzten Quellwerte zu verfolgen. **Committen Sie diese Datei**, damit alle Entwickler dieselbe Übersetzungsbasis (Baseline) teilen.
...
Rosetta erstellt ein `.rosetta/`-Verzeichnis im Stammverzeichnis Ihres Projekts für den internen Status. Sie sollten dies im Allgemeinen **zu `.gitignore` hinzufügen** — es handelt sich um lokale Optimierungen, nicht um Projektquellen:
...
|------|---------|--------|
| `tm.json` | Translation Memory-Cache — speichert vorherige Übersetzungen, verschlüsselt nach Quelltext + Gebietsschema (Locale) + Methode | Nein (lokaler Cache) |
| `xliff/*.xliff` | XLIFF-Exportdateien für die Überprüfung durch professionelle Übersetzer | Nein (flüchtig) |
| `methods/` | Installierte Methoden-Plugin-Manifeste | Ja (gemeinsame Konfiguration) |
...
### Benutzerdefinierte Anbietererweiterung

Erweitern Sie `DirectLLMMethod`, um in ca. 40 Zeilen einen neuen LLM-Anbieter hinzuzufügen:
