...
I-run ang `i18n-rosetta <command> --help` para sa detalyadong help sa kahit anong command.
...
**`--langs` option**: Comma-separated list ng mga target language codes. Ini-skip nito ang language prompt at ina-apply ang default register presets para sa bawat language. I-combine sa `--yes` para sa fully non-interactive setup.
...
Tina-translate ang mga missing at stale keys sa lahat ng locale files. Nagra-run po ito ng post-sync verification by default.
...
**Translation Memory**: By default, nilo-load ng `sync` ang `.rosetta/tm.json` at sine-serve ang mga cached translations para sa mga unchanged source values. Gamitin ang `--no-tm` para i-bypass ang cache (useful ito kapag nag-switch ng translation providers o nagde-debug ng quality). Tingnan ang [Translation Memory](/docs/concepts/translation-memory).

**Change detection**: Nag-i-store ang rosetta ng SHA-256 hashes sa `.i18n-rosetta.lock`. Kapag nagbago ang source values, awtomatikong ire-re-translate ng susunod na sync ang mga keys na iyon. I-commit ang lock file para ma-share ng lahat ng developers ang baseline.

**Parallelism**: Parehong nagra-run in parallel ang JSON key translation at content translation. Sabay-sabay na tina-translate ang mga JSON locales (default: 200 concurrent locales), at naka-parallelize din ang mga batches sa loob ng bawat locale (4 concurrent batches). Ang content translation (Markdown, MDX, blog posts) ay nagra-run sa isang flat work-item pool (default: 48 concurrent API calls). I-override gamit ang `--json-concurrency`, `--content-concurrency`, o `--concurrency` (sine-set pareho).
...
Nag-u-update in-place ang mga progress bars pagkatapos ng bawat batch (~80 keys). Gamitin ang `--quiet` para sa errors/warnings lang, o `--json` para sa machine-readable NDJSON output. Pareho nitong sinu-suppress ang progress bar at banner.

---

## watch

Nag-a-auto-sync kapag nagbago ang source locale file. Nagra-run ito hanggang ma-interrupt gamit ang `Ctrl+C`.
...
Inililista ang lahat ng untranslated na `[EN]`-prefixed fallback values mula sa mga nakaraang runs. Nag-e-exit ito with code 1 kapag may nahanap — gamitin ito bilang CI gate para i-fail ang mga builds na may incomplete translations.
...
Nire-re-read nito ang lahat ng locale files mula sa disk at vine-verify kung present at correct ang mga translations. Ito rin ang same verification na awtomatikong nagra-run sa dulo ng bawat `sync` (maliban na lang kung ipinasa ang `--no-verify`).
...
- `[EN]` fallback markers mula sa mga nakaraang runs
- Empty translations
- Script compliance — dapat may non-ASCII translations ang mga non-Latin locales
- Placeholder preservation — nagma-match sa source ang mga ICU placeholders
- Encoding issues — BOM markers, invisible characters
- Source echoes — mga values na identical sa source (warning)

---

## lint

Ini-scan ang source code para sa mga hardcoded user-facing strings na dapat gumagamit ng i18n translation calls. Nag-a-auto-detect ito ng framework niyo (next-intl, react-i18next, vue-i18n, Hugo).
...
**Exclusions**: Gumawa ng `.rosettaignore` sa inyong project root (glob patterns, tulad ng `.gitignore`).

---

## wrap

Ina-auto-wrap ang mga hardcoded strings na na-detect ng `lint` sa loob ng `t()` calls. Gumagawa po ito ng automatic backups bago i-modify ang mga files.
...
**Safety gates:**
1. Git-clean check (naka-skip sa dry-run)
...
**Mga tsine-check nito:**
- Placeholder corruption (hal., present ang `{name}` sa source pero missing sa target)
...
- ICU MessageFormat plural category completeness (hal., kailangan ng Arabic ng 6 categories)

---

## tm

I-manage ang Translation Memory cache (`.rosetta/tm.json`). Nag-i-store ang TM ng mga previous translations at sine-serve ang mga ito sa mga susunod na syncs imbes na tawagin ang API.
...
Tingnan ang [Translation Memory](/docs/concepts/translation-memory) para malaman kung paano gumagana ang TM at kung kailan ito dapat i-clear.
...
I-manage ang mga translation method plugins. Ang mga plugins ay mga pre-packaged translation recipes na naka-install sa `.rosetta/methods/`.
...
Nagda-download at nagma-manage ng PUA web fonts para sa mga constructed language script converters. Ang mga languages na gumagamit ng Private Use Area characters (Klingon, Sindarin, Kryptonian) ay kailangan ng custom web fonts para ma-render ang kanilang mga scripts. Dina-download ng command na ito ang mga iyon mula sa mga verified open-source repositories.
...
| `--config <path>` | Path sa config file (ginagamit para ma-detect kung aling languages ang kailangan ng fonts) |

**Auto-detection:** Ang output directory ay ini-infer mula sa inyong project structure:
...
| **Verify** | `verify` | Post-sync / CI | I-confirm kung present at correct ang mga translations |
| **Audit** | `audit` | Build step | I-fail ang deployment kapag may `[EN]` markers ang kahit anong locale |
