...
- **Version banner**: Ipinapakita ang installed version para sa debugging at issue reports.
- **Format detection**: Nire-report ang file format at kung ito ba ay na-auto-detect `(auto)` o explicitly na na-configure `(config)`. Supported ang `json`, `toml`, at `yaml`.
...
Binabasa ng Rosetta ang `.i18n-rosetta.lock`, na nag-i-store ng SHA-256 hashes ng mga previously translated na source values. Para sa bawat key, chine-check nito ang:
...
Kaya po tina-translate lang ng rosetta kung ano ang nagbago — hindi nito nire-re-translate ang buong file niyo sa bawat sync.

### 4. Batching

Ginu-group ang mga keys sa mga batches (default: 80 keys/batch para sa LLM, 128 para sa Google Translate). Nakakabawas ang batching sa mga API round trips habang pinapanatiling manageable ang mga prompts.
...
Bago mag-batching, chine-check ng rosetta ang Translation Memory cache (`.rosetta/tm.json`). Ang mga keys na may source text + locale + method na nagma-match sa previous translation ay sine-serve agad mula sa cache — hindi na kailangan ng API call.
...
Ang TM ang primary cost-saving mechanism. Ang pag-re-run ng sync pagkatapos ng isang key change ay ita-translate lang ang isang key na iyon, hindi ang buong file. Tingnan ang [Translation Memory](/docs/concepts/translation-memory) para sa details.

Para ma-bypass ang cache para sa isang run: `i18n-rosetta sync --no-tm`

### 5. Translation

Ang bawat batch ay ipinapadala sa naka-configure na translation method:
...
Ang system message (register, gender guidance, rules) ay identical across batches para sa isang given locale, kaya nai-enable ang **prompt caching** — kina-cache ng mga providers tulad ng Anthropic at Google ang mga repeated system messages, na nakakabawas sa token costs.

### 6. Quality Gate

Bawat translation ay vina-validate bago ito i-write sa disk. May limang checks na nagra-run:
...
Para sa mga coached pairs na may dictionary, chine-check ng rosetta kung ginamit ba talaga ng LLM ang required terminology pagkatapos ng translation. Ang mga violations ay nalo-log bilang `[TERM]` warnings:
...
Kapag may JSON parse failure o batch-level errors, nagre-retry ang rosetta gamit ang progressively smaller batches:
...
Pagkatapos ma-process ang lahat ng pairs, muling binabasa ng rosetta ang mga isinulat na locale files mula sa disk at nagra-run ng verification pass (maliban kung naka-set ang `--no-verify`). Nahuhuli nito ang gap sa pagitan ng sync reporting success at kung mali ba talaga ang mga keys:

- **Key parity** — lahat ng source keys ay present sa bawat target
- **`[EN]` fallback markers** — legacy markers mula sa mga prior runs
- **Empty translations** — blank values na nakalusot
...
Available din ito bilang isang standalone `i18n-rosetta verify` command para sa CI gates.
...
2. Para sa bawat file × locale pair, chine-check nito ang isang hiwalay na content lock file (`.i18n-rosetta-content.lock`) para sa SHA-256 hash changes
3. Kinokolekta ang mga nagbago o nawawalang files sa isang flat work-item pool
4. Pinu-process ang pool nang may **parallel concurrency** (default: 12 simultaneous API calls)
...
Pareho na ngayong nagra-run in parallel ang Phase 1 (JSON keys) at Phase 2 (content):

- **Phase 1**: Sabay-sabay na nagfa-fire ang lahat ng locale translations (default: 50 simultaneous locales). Sa loob ng bawat locale, nagra-run din in parallel ang mga API batches (4 concurrent batches). Ang isang 12-locale sync na may 120 keys ay nakukumpleto sa loob ng ~1 minuto imbes na ~15 minuto.
...
I-control ang parallelism gamit ang `--json-concurrency`, `--content-concurrency`, o `--concurrency` (sine-set ang pareho):
...
Habang nagta-translate, pinu-protektahan ng rosetta ang mga non-translatable content:

- **Code blocks** (fenced at indented) ay pinapalitan ng mga placeholders
- **Frontmatter** fields na wala sa `translatableFields` list ay pini-preserve as-is
- **Links**, image paths, at HTML tags ay protektado
- **Shortcodes** at interpolation variables (hal., `{count}`, `{{.Params.title}}`) ay naka-shield

Pagkatapos ng translation, nire-restore at vina-validate ang lahat ng placeholders. Kung may nawawala o corrupt, ire-reject ang translation at ire-retry.

## Partial Success

Hindi bina-block ng isang failed batch ang iba pa. Kung 9 out of 10 batches ang nag-succeed, isusulat ang 9 na iyon. Ilo-log ang failed batch, at pwede niyo pong i-re-run ang `sync` para mag-retry.
...
Bago mag-translate, nagge-generate ang rosetta ng **pre-sync cost report** na nagpapakita ng estimated costs per pair. Awtomatiko itong nagra-run sa bawat `sync` — makikita niyo po ito bago pa man gumawa ng anumang API calls.
...
Kapag hindi ma-determine ng isang method ang cost (LLM methods, remote APIs), nagre-report ang rosetta ng `—` imbes na manghula. Gamitin ang `--dry` para makita ang cost estimates nang hindi pa talaga nagta-translate.
