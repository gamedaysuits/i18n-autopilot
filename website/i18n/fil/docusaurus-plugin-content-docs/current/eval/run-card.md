...
Tinu-tukoy nito ang evaluation dataset at naka-pin ito sa isang specific na content version via SHA-256.
...
| `sha256` | `string` | SHA-256 hash ng dataset file contents. Sini-siguro nito ang eksaktong data na ginamit |
...
Isang reproducibility identifier. Ang dalawang runs na may identical na fingerprints ay gumamit ng parehong experimental setup.

| Field | Type | Description |
|-------|------|-------------|
| `hash` | `string` | SHA-256 hash ng sorted components |
| `components` | `object` | Ang input values na nai-hash |
...
Tinu-tukoy ng fingerprint ang *experiment configuration*. Vini-verify naman ng `run_card_hash` ang *result file integrity*. Tingnan po ang [Fingerprint vs Run Card Hash](/docs/eval/harness#fingerprint-vs-run-card-hash) para sa detalye.
:::
...
| `exact_matches` | `number` | Entries kung saan ang output ay exactly matched sa gold standard |
...
Scores na naka-break down by difficulty tier. Ang bawat key (`easy`, `medium`, `hard`) ay naglalaman ng parehong metric fields tulad ng top-level scores.
...
Scores na naka-break down by entry provenance. Ang bawat key (hal., `gold_standard`, `textbook`) ay naglalaman ng parehong metric fields.
...
| `cached_tokens` | `number` | Tokens na na-serve mula sa prompt cache ng provider |
| `total_cost_usd` | `number` | Total cost in USD (tulad ng nai-report ng API) |
...
| Field | Type | Description |
|-------|------|-------------|
| `harness_version` | `string` | Harness version (ka-mirror ng top-level `harness_version`) |
| `harness_git_commit` | `string` | Git commit SHA ng harness at run time |
...
Ang per-entry results array. Isang object per dataset entry, in index order.

| Field | Type | Description |
|-------|------|-------------|
| `entry_index` | `number` | Index ng entry na ito sa dataset (nag-ma-match sa `entries[].index`) |
| `source_text` | `string` | Ang source text na nai-translate |
...
| `error` | `string \| null` | Error message kung nag-fail ang entry na ito. `null` kapag success |
...
Ito ang tamper-detection seal. Nire-recompute ng leaderboard ang hash na ito upon submission at nire-reject ang mga cards na hindi nag-ma-match.

**Pag-compute ng hash:**

1. I-serialize ang run card sa JSON na may `run_card_hash` na naka-set sa `""`
