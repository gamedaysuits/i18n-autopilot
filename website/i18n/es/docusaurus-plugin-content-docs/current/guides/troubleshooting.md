...
**"not found in available models"** — El modelo puede estar obsoleto o mal escrito. Rosetta obtiene la lista de modelos en vivo del proveedor y sugiere alternativas. Revise la documentación del proveedor para conocer los nombres de los modelos actuales.
...
La verificación de cumplimiento de escritura del control de calidad detecta la mayoría de los casos. Si persiste:
...
1. **Reduzca el tamaño del lote** — Los lotes más pequeños producen resultados más enfocados
...
Rosetta detecta automáticamente los archivos de configuración regional (locale). Si no puede encontrarlos:

1. **Revise `localesDir`** — Debe apuntar al directorio que contiene los archivos de configuración regional:
   ```json
   { "localesDir": "./locales" }
   ```
2. **Revise el nombre de los archivos** — Los archivos deben nombrarse por el código de configuración regional: `en.json`, `fr.json`, etc.
...
:::warning
Eliminar el archivo de bloqueo significa que la próxima sincronización volverá a traducir todas las claves, no solo las modificadas. Esto tiene implicaciones en el costo de la API para proyectos grandes.
:::
...
Rosetta traduce todas las configuraciones regionales en paralelo de forma predeterminada. Si la sincronización sigue siendo lenta:
...
2. **Aumente el tamaño del lote** (el valor predeterminado es 80):
   ...
   { "batchSize": 120 }
   ```
3. **Ajuste la concurrencia** — El paralelismo de la configuración regional JSON es de 200 por defecto y el del contenido es de 48. Si su proveedor de API admite límites de tasa más altos:
   ```bash
...
- **Use la Memoria de Traducción (TM)** — La TM está activada por defecto. Ejecute `i18n-rosetta tm stats` para verificar que esté funcionando. Si ve 0 entradas después de múltiples sincronizaciones, algo puede estar mal con los permisos de su directorio `.rosetta/`
- **Use el almacenamiento en caché de prompts (prompt caching)** — Rosetta divide los mensajes del sistema/usuario para obtener aciertos de caché en los modelos de Anthropic y Google
- **Use Google Translate para idiomas de Nivel 2** — Consulte la guía práctica [Traducir 30 idiomas](/docs/tutorials/translate-30-languages)

### Traducciones obsoletas después de cambiar de proveedor

Si cambia de un método de traducción a otro (por ejemplo, de `llm` a `deepl`), la caché de la TM aún puede servir traducciones antiguas del método anterior para las claves cuyo texto de origen no ha cambiado. La clave de la caché incluye el nombre del método, por lo que la mayoría de los casos se manejan automáticamente. Pero si cambió `model` dentro del mismo método:
...
Consulte [Memoria de traducción](/docs/concepts/translation-memory) para obtener detalles sobre el diseño de la clave de caché.

## ¿Aún tiene problemas?

- **[Problemas en GitHub (Issues)](https://github.com/gamedaysuits/i18n-rosetta/issues)** — Busque problemas existentes o abra uno nuevo
- **[Documentación de arquitectura](/docs/concepts/architecture)** — Comprenda el diseño del sistema
- **[Control de calidad (Quality Gate)](/docs/concepts/quality-gate)** — Cómo funciona la validación internamente