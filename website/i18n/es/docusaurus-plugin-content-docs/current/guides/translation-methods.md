...
**Cómo funciona:**
1. Agrupa claves en lotes (30 por lote de forma predeterminada) con instrucciones de registro y contexto
...
2. Inyecta reglas gramaticales, términos del diccionario y notas de estilo en el prompt del sistema
...
**Cuándo usarlo:** Idiomas de bajos recursos, terminología específica de un dominio (legal, médico), registros formales o cualquier caso en el que el resultado genérico del LLM no sea lo suficientemente preciso.
...
Traduce directamente a través de la API Chat Completions de OpenAI. Sin OpenRouter como intermediario: su clave, su cuenta, su panel de uso.
...
Traduce directamente a través de la API Messages de Anthropic. Utiliza el parámetro `system` para los datos de entrenamiento, lo que habilita el almacenamiento en caché de prompts (prompt caching) de Anthropic.
...
- ✅ Nivel gratuito (generosa cuota diaria)
- ✅ Retroceso exponencial con reintentos
...
Los proveedores directos de LLM (`openai`, `anthropic`, `gemini`) validan la cadena de su modelo en el primer uso. Esto detecta tres categorías de errores:

**Formato de método incorrecto**: usar una ruta de modelo al estilo OpenRouter con un proveedor directo:
...
**Modelo obsoleto o mal escrito**: en la primera llamada a la API, rosetta obtiene la lista de modelos en vivo del proveedor y verifica su modelo contra ella:
...
**Limitaciones:**
- ⚠️ **No es compatible con Markdown.** Corromperá bloques de código, shortcodes y variables de interpolación.
...
**Cuándo usarlo:** Idiomas europeos en los que DeepL destaca (alemán, francés, español, holandés, polaco, etc.). El soporte de glosarios aplica una terminología consistente sin necesidad de datos de entrenamiento.
...
**Cuándo usarlo:** Proyectos que requieren traducción sin conexión, cumplimiento de privacidad de datos (GDPR) u operación sin costo. Especialmente útil para pipelines de CI que no deberían depender de APIs externas.

**Características:**
- ✅ Autohospedado: sin llamadas a APIs externas
...
Un cliente HTTP ligero para endpoints de traducción alojados por la comunidad o protegidos por propiedad intelectual (IP). Rosetta envía las claves y recibe las traducciones de vuelta; no contiene ninguna lógica de traducción.
...
¿Va a cambiar de método? El formato del modelo y la variable de entorno cambian; aquí tiene el mapa:
...
- Si usa el formato de modelo incorrecto, rosetta le advertirá; consulte [Validación de modelos](#model-validation).
...
**Use OpenRouter** cuando desee cambiar entre modelos sin modificar las variables de entorno, o cuando desee acceder a más de 200 modelos desde una sola clave. **Use proveedores directos** cuando desee una facturación más sencilla, menor latencia (sin intermediarios) o acceso a funciones específicas del proveedor, como el almacenamiento en caché de prompts de Anthropic.
:::
...
| `google-translate` | ~$0.02 | La más rápida | Adecuada | Alto volumen, idiomas europeos |
| `deepl` | ~$0.02 | Rápida | Buena | Idiomas europeos, terminología |
| `microsoft-translator` | ~$0.01 | Rápida | Adecuada | Entornos de Azure, amplia cobertura de idiomas |
| `libretranslate` | **Gratis** (autohospedado) | Varía | Aceptable | Entornos aislados (air-gapped), GDPR, pipelines de CI |
...
| `llm` (OpenRouter) | Varía según el modelo | Varía | Varía | Comparación de modelos, experimentación |

:::note Estas son estimaciones
Los costos reales dependen de la longitud del texto de origen, el tamaño del lote y los cambios de precios del proveedor. Consulte la página de precios actual de cada proveedor para conocer las tarifas exactas.
:::
