...
El bloque de configuración `typegen` es reconocido y conservado por el cargador de configuración, pero la generación de tipos de TypeScript aún no está implementada. Este es un marcador de posición para una función planeada. Configurar estos valores no tiene ningún efecto.
:::
...
| `localesDir` | `string` | `"./locales"` | Ruta a los archivos de configuración regional. Rosetta escanea este directorio. |
| `contentDir` | `string` | `null` | Directorio de contenido de Hugo. Habilita la traducción del cuerpo de Markdown. |
| `translatableFields` | `string[]` | `null` | Anula los campos predeterminados del frontmatter traducible para la traducción de contenido. `null` utiliza los valores predeterminados integrados (`title`, `description`, `summary`). |
...
| `defaultMethod` | `string` | `"llm"` | Método de traducción predeterminado: `llm`, `llm-coached`, `google-translate`, `deepl`, `microsoft-translator`, `libretranslate`, `openai`, `anthropic`, `gemini`, `api`. Se anula con el flag de CLI `--method`. |
| `batchSize` | `number` | `30` | Claves por lote de traducción. Mayor cantidad = menos llamadas a la API, pero prompts más grandes. |
| `concurrency` | `number` | `12` | Máximo de llamadas paralelas a la API para la traducción de contenido (Markdown/MDX). Se anula con el flag de CLI `--concurrency`. |
| `fallbackPrefix` | `string` | `"[EN] "` | Prefijo que se añade a los valores de respaldo no traducidos. Utilizado por `audit` para detectar traducciones incompletas. |
...
Rosetta comprueba si la cadena coincide con una clave preestablecida en la tarjeta del idioma. Si es así, se utiliza el prompt de registro completo de la tarjeta. Si no, la cadena se utiliza tal cual. Consulte [Idiomas compatibles](/docs/reference/supported-languages#language-cards) para ver los preajustes disponibles.
...
| `name` | `string` | Nombre del idioma legible por humanos (para la visualización de estado) |
...
Por ejemplo, si `pairs["en:fr"]` establece `model`, anula tanto los valores de `model` a nivel de idioma como los globales.
:::

## Origen distinto del inglés
...
Rosetta crea `.i18n-rosetta.lock` para rastrear los hashes SHA-256 de los valores de origen traducidos. **Haga un commit de este archivo** para que todos los desarrolladores compartan la misma base de traducción.
...
Rosetta crea un directorio `.rosetta/` en la raíz de su proyecto para el estado interno. Por lo general, **debería agregar esto a `.gitignore`**; es una optimización local, no código fuente del proyecto:
...
| `xliff/*.xliff` | Archivos de exportación XLIFF para la revisión de traductores profesionales | No (transitorio) |
...
Extienda `DirectLLMMethod` para agregar un nuevo proveedor de LLM en aproximadamente 40 líneas:
...
- [Especificación de plugins](/docs/reference/plugin-spec) — formato del manifiesto del plugin de método
