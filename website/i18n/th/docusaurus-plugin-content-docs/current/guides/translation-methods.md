...
**วิธีการทำงาน:**
1. จัดกลุ่มคีย์ (ค่าเริ่มต้น 80 คีย์/ชุด) พร้อมคำสั่งเกี่ยวกับระดับภาษา (register) และบริบท
2. ส่งไปยัง OpenRouter ในรูปแบบ prompt ที่มีโครงสร้าง
3. แปลงผลลัพธ์ (Parse) การตอบกลับแบบ JSON
4. ตรวจสอบความถูกต้องของการแปลแต่ละรายการผ่าน [quality gate](/docs/concepts/quality-gate)
5. บันทึกคำแปลที่ผ่านเกณฑ์, ลองใหม่ หรือปฏิเสธคำแปลที่ไม่ผ่านเกณฑ์
...
**วิธีการทำงาน:**
1. โหลดข้อมูล coaching จาก `.rosetta/coaching/<locale>.json` หรือไดเรกทอรี `coaching/` ของปลั๊กอิน
2. แทรกกฎไวยากรณ์, คำศัพท์ในพจนานุกรม, และบันทึกรูปแบบลงใน system prompt
3. คำศัพท์ในพจนานุกรมที่ตรงกับคีย์ต้นทางจะถูกรวมไว้เป็นคำศัพท์ที่บังคับใช้
...
- ✅ ระดับใช้งานฟรี (โควตารายวันจำนวนมาก)
- ✅ การหน่วงเวลาแบบทวีคูณพร้อมการลองใหม่
...
**คุณสมบัติ:**
- ✅ การตรวจจับ endpoint แบบ free/pro อัตโนมัติ (ต่อท้ายด้วย `:fx` บนคีย์ฟรี)
- ✅ การสร้างและการจัดการ Glossary
...
ดู [การสนับสนุนภาษาที่มีทรัพยากรน้อย (Support a Low-Resource Language)](https://mtevalarena.org/docs/community/low-resource-languages) สำหรับคำแนะนำการโฮสต์โดยชุมชนแบบเต็มรูปแบบ, และ [การให้บริการวิธีการผ่าน API (Serving a Method via API)](/docs/guides/serving-a-method) สำหรับข้อกำหนดของ endpoint
:::
...
ปลั๊กอินที่พัฒนาและพิสูจน์แล้วใน [eval harness](https://mtevalarena.org/docs/specifications/harness) สามารถติดตั้งได้โดยตรง — วิธีการที่คุณตรวจสอบความถูกต้องที่นั่นสามารถนำมาใช้งานที่นี่ได้ด้วยคำสั่ง `plugin install` เพียงคำสั่งเดียว ดู [การประเมิน MT (MT Evaluation)](https://mtevalarena.org/docs/leaderboard/rules) สำหรับเวิร์กโฟลว์การประเมินฉบับเต็ม
:::
...
ดู [ข้อกำหนดของปลั๊กอิน (Plugin Specification)](/docs/reference/plugin-spec) สำหรับรูปแบบ manifest ฉบับเต็ม
...
- ผู้ให้บริการโดยตรงแต่ละรายมีตัวแปรสภาพแวดล้อมของตนเอง (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`)
- หากคุณใช้รูปแบบโมเดลผิด rosetta จะเตือนคุณ — ดู [การตรวจสอบความถูกต้องของโมเดล (Model Validation)](#model-validation)
...
ต้นทุนจริงขึ้นอยู่กับความยาวของข้อความต้นทาง, ขนาดชุดข้อมูล (batch size), และการเปลี่ยนแปลงราคาของผู้ให้บริการ โปรดตรวจสอบหน้าราคาปัจจุบันของผู้ให้บริการแต่ละรายสำหรับอัตราที่แน่นอน
:::
...
- [ข้อกำหนดของปลั๊กอิน (Plugin Specification)](/docs/reference/plugin-spec)
