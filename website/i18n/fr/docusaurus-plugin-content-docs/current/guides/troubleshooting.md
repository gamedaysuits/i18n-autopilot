...
Rosetta détecte automatiquement les fichiers de paramètres régionaux. S'il ne parvient pas à les trouver :

1. **Vérifiez `localesDir`** — Doit pointer vers le répertoire contenant les fichiers de paramètres régionaux :
   ```json
   { "localesDir": "./locales" }
   ```
...
Rosetta traduit les paires de manière séquentielle par défaut. Pour accélérer les synchronisations multilingues :

1. **Utilisez Google Translate pour les paires à fort volume** — C'est 10 à 50 fois plus rapide que la traduction par LLM.
2. **Augmentez la taille des lots** (jusqu'à 50, la valeur par défaut est 30) :
   ```json
   { "batchSize": 50 }
   ```
3. **Utilisez un modèle rapide** — `gpt-4o-mini` est considérablement plus rapide que `gpt-4o`.
